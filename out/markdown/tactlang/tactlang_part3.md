...
CrazyIsland: Hi Guys, I've try standard simple counter contract —— message Add {     amount: Int as uint32; }  contract TestTonNew(     // Persistent state variables declared via the     // contract parameters syntax, which was introduced in v1.6.0     //     // See: https://docs.tact-lang.org/book/contracts/#parameters     id: Int as uint32,     counter: Int as uint32, ) {     // Empty receiver for the deployment,     // which expects the null message body     receive() {         // Forward the remaining value in the         // incoming message back to the sender         cashback(sender());     }      receive(msg: Add) {         self.counter += msg.amount;          // Forward the remaining value in the         // incoming message back to the sender         cashback(sender());     }      get fun counter(): Int {         return self.counter;     }      get fun id(): Int {         return self.id;     } }  —— and try to use this code for payload —-  createAddPayload(amount) {     // Для контракта TestTonNew с message Add { amount: Int as uint32; }     // Op code для Add вычисляется как crc32("Add") & 0x7fffffff          const body = beginCell()       .storeUint(0x7362d09c, 32) // op code для Add message       .storeUint(0, 64) // query id       .storeUint(amount, 32) // amount как uint32       .endCell();          console.log('Создан payload для Add:', {       opCode: '0x7362d09c',       queryId: 0,       amount: amount,       boc: body.toBoc().toString('base64'),       hex: body.toBoc().toString('hex')     });          return body.toBoc().toString('base64');   }   —— but I got error (exit code ) 130 all the time. IS anybody know how to fix it? Many thanks.

Nerses: here is the reason why you get such an error (reply to 66218)

fruitful-l: Try declaring message as  message(0x7362d09c) Add (reply to 66218)

fruitful-l: In the contract

&rey: specifically, this is false (reply to 66218)

CrazyIsland: Ok, but how to make simple contract (generated by blueprint ) works? Or blueprint generates unworkable code? (reply to 66221)

fruitful-l: The deployment script from here definitely is not generated by blueprint (reply to 66218)

CrazyIsland: I have try it in many ways, but it was unworkable all the time, so question is how to make it work? (reply to 66222)

&rey: You need the TL-B scheme of message (without explicit opcode of course); remove parentheses and calculate its crc32. &0x7FFF_FFFF is right. (reply to 66225)

Cornelius: Hello,  I'm looking for some guidance regarding NFTs. I have a few questions that I couldn't resolve through the official documentation.  My main issue is that I don't fully understand how to create an NFT. From what I gather, it seems to be essentially another smart contract. However, I'm unsure about the correct approach for transferring ownership: Should I use a transfer function, or is it necessary to create a new NFT altogether?  Additionally, I'm a bit confused about the default or standard attributes of an NFT. I remember seeing them written in snake_case, but based on Tact's style conventions, shouldn't they be in camelCase instead?  Any clarification or resources you could share would be greatly appreciated
...
Jason 🆙 UXUY $ETN: Hello everyone glad I found this group and having some problems trying to create a smart contract for an nft I've tried to adapt the standard in various ways to meet our use cases is there anyone here who could spare a few minutes to help me out

akki: Don't ask for it() Just go for it()  fun it() {return help;} (reply to 70417)

Jason 🆙 UXUY $ETN: Okay so I'm trying to create a collection which is similar to the telemint smart contract minus the DNS resolution so basically a collection which is minted on demand of @ usernames with rating based on vanity AKA three digits four digits five digits and more have different prices and I'm trying to get it to be using a specific Jetton Master as payment have tried various different iterations of the code I can't even get it to compile (reply to 70418)

Jason 🆙 UXUY $ETN: Are we allowed to share GitHub repository links in this group

Jason 🆙 UXUY $ETN: Is any of this even remotely correct  () deploy_item(int item_index, int amount, cell item_content) impure { cell data = get_data(); slice ds = data.begin_parse(); force_owner(ds~load_msg_addr()); ;; Check that the message is from the owner ds.skip_bits(64); ;; next_item_index slice collection_content = ds~load_ref().begin_parse(); cell item_code = ds~load_ref(); slice royalty_params_slice = ds~load_ref().begin_parse(); ;; --- ETN ECOSYSTEM CONFIGURATION --- ;; Define the hardcoded addresses and ETN Jetton master address. slice beneficiary_address = "UQDqkn-Y1G77_un13nqgx1jGh6XXH6deL737gdUxlpFhz9eF"a; slice royalty_address = "UQA7KhoTClG9QR7DwN1RGAkWIRU0dNp_R8F615y5PiEGQDEK"a; slice etn_jetton_master_address = "EQAz_XrD0hA4cqlprWkpS7TIAhCG4CknAfob1VQm-2mBf5Vl"a; ;; Configure the auction parameters for the new item. cell auction_config = begin_cell() .store_slice(beneficiary_address) .store_coins(from_nano(100, 9)) ;; Example: initial min bid of 100 ETN (assuming 9 decimals) .store_coins(from_nano(1000000, 9)) ;; Example: max bid .store_coins(from_nano(10, 9)) ;; Example: bid step of 10 ETN .store_uint(now() + 86400, 32) ;; Example: auction ends in 24 hours .end_cell(); ;; Configure the royalty parameters. ;; The royalty_params cell will now store the ETN Jetton master address as its first element. royalty_params_slice~load_uint(16); ;; skip original numerator royalty_params_slice~load_uint(16); ;; skip original denominator int royalty_numerator = royalty_params_slice.preload_uint(16); int royalty_denominator = royalty_params_slice.preload_uint(16); cell royalty_params = begin_cell() .store_slice(etn_jetton_master_address) .store_slice(royalty_address) .store_uint(royalty_numerator, 16) .store_uint(royalty_denominator, 16) .end_cell(); cell item_data = begin_cell() .store_slice(my_address()) .store_uint(item_index, 64) .store_slice(my_address()) ;; Initial owner is the collection itself .store_ref(item_content) .store_maybe_ref(null()) ;; No auction state initially .store_ref(auction_config) .store_ref(royalty_params) .end_cell(); int flags = 0; send_deploy(calculate_contract_address(item_code, item_data), amount, item_code, item_data, flags); } This modification ensures that every username NFT minted by this collection is correctly configured to use the ETN Jetton for payments and royalties, with all funds directed to the specified ecosystem addr

Jason 🆙 UXUY $ETN: So essentially this is one part of what we're trying to write I tried to send the markdown file which is basically our PRD but the group had been bought wouldn't let me so I know everybody in here is busy and have their own projects I'm not a very good developer in general and I seem to be worse when it comes to blockchain based things so I would really appreciate any help if anyone can help me or has any information as to where I'm going wrong you can DM me if you need

Es Ist Das: Does somebody knows, how to read map<Int, Int> from Cell?  // Works properly let content: map<Int, Int> = emptyMap(); content.set(1, 1); self.storage = content.asCell();  // Can't read cell as map let fromMap: map<Int, Int> = self.storage;
...
okolo: Hi there! i have have an init in the contract that takes variables. When I wrote the contract, I assumed these variables would be set during deployment: contract WalletContract {   owner: Address;   paused: Bool;   balances: map<Address, Int as coins>;   jettonMaster: Address;   withdrawFeeBps: Int;    init(ownerAddr: Address, jettonMasterAddr: Address, withdrawFeeBps: Int) {     self.owner = ownerAddr;     self.paused = false;     self.balances = map<Address, Int as coins> {};     self.jettonMaster = jettonMasterAddr;     self.withdrawFeeBps = withdrawFeeBps;   }  I assumed these variables could be passed in the deployment script: export async function deployWallet(provider: NetworkProvider, owner: string, jettonMaster: string) {   const wallet = provider.open(     await Wallet.fromInit({       ownerAddr: new Address(owner),       jettonMasterAddr: new Address(jettonMaster),       withdrawFeeBps: 100, // 1%     })   );  however, fromInit points to the .ts file generated by the build, and there’s no place there to pass the variables I need: static async fromInit() {     const __gen_init = await Wallet_init();     const address = contractAddress(0, __gen_init);     return new Wallet(address, __gen_init); }  what is the correct way to pass these variables ownerAddr and jettonMasterAddr? i have no prior experience and couldn’t find clear information — do I need to hardcode them directly in the contract? Pls help)
...
Shyzik: И где мое вознаграждения (reply to 75058)

Shyzik: Ебу вы меня взяли на лямы

Shyzik: Я вам поверил вернул все своим владельцам и даже больше ребята побойтесь бога и людей которые пришли с войны

Vlad: ?

Vlad: Что за нашествие на тоновские чаты... Кстати это вроде англоязычный чат

Shyzik: Слышиш у меня вошли медернезаторы вьебали пароли ко всем соц сетям банкам онлайн и оставили меня без копейки и ещё крипто кредитов на меня и на мою маму набрали вы вообще там страх в Польше потерялии
...
Stunning sky: hi guys

Stunning sky: gud mrng (reply to 81274)

— 2026-03-29 —

Popeye: Hey (reply to 81273)
...
Батч (кодер): Лучший хостинг проект с работой 24/7 @BlimbiSaferobot OPEN скидка на любой тариф/сервер 20%

Alividerchibucefal: Ник бота имба,I like it (reply to 81852)

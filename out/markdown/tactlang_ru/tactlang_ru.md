...
klass: Ты здесь amount неправильно вытаскиваешь как будто бы:  self.myJettonBalance += msg.amount; (reply to 2083)
...
klass: Да, Amount это именно сами TON, которые прикреплены к нотификейшену 👍 (reply to 2086)
...
klass: Вопрос. Почему из пейлоада опкод, а не из msg? (reply to 2096)

Daniil: ну сначала идет оп код JettonTransferNotification а потом уже 0x54d5c342 в forwardPayload (reply to 2097)

klass: Понял, думаю. А ошибки выдает какие-нибудь или что там? (reply to 2098)
...
klass: На каком этапе? Поставил Verbosity? (reply to 2100)
...
klass: Предлагаю поставить подробный дебаг с помощью этого кода, это покажет из какого конкретно слайса не читает. Подставь там нужный адрес в тестах и весь этот код перед выполнением теста  await blockchain.setVerbosityForAddress(orderNft.address, {     vmLogs: 'vm_logs_full',     print: true,     blockchainLogs: true,     debugLogs: true }) (reply to 2104)

klass: В базом понимании ты пытаешься читать из слайса то, чего там нет. Битов банально этих нету

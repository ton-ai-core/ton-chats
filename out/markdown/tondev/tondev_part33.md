...
RootTon: Или ton API /trace (reply to 236483)
...
RootTon: Всем привет, можно ли как то передать комментарий к транзакции за жетоны? потому что я вставлял storestringtail после storeref и в тонвивере ничего не отобразилось и хотел бы узнать есть ли какие то варианты или нельзя этого сделать?
...
RootTon: это получается на уровне смарта эти 32 бита потом еще скипать придется? (reply to 304757)

RootTon: потому что у меня ж форвард пейлоад это словарь и просто туда докинуть storestringtail я скорее всего не смогу?

&rey: Комментарий же занимает место любого другого форварда или тела сообщения.

RootTon: дак там же 0,32 тоесть 1ые 32 бита будут нулевыми, а в смарте у меня идет первым делом чтение списка и типо нужно ли мне будет делать skip_bits(32) в inner_payload или же нет? (reply to 304760)

RootTon: ну точнее там будет 0 в 32ух битной системе (вроде так корректнее) (reply to 304761)
...
RootTon: аааааа даже так (reply to 304763)

RootTon: понял спасибо в таком случае поживу без комментариев :( (reply to 304763)

RootTon: а можете посоветовать может какой нибудь базовый стандарт валидации транзы в тон? просто у меня был модуль валидации транз который работал на основе комментариев и boc чтобы найти нужныю транзу в трейсах, но тк я не могу сделать тут комент мне нужен иной способ и пока в голову ничего не приходит, искать транзу только по boc немного бесполезно ибо юзер может не вернуться в приложение и бока я не получу на бэк соответственно (reply to 304763)

Denis: Классику читали? https://gist.github.com/mr-tron/a4b3e8eff3a982e3426f6397eb9a2450 (reply to 304766)

RootTon: Щас заполню эту галочку ) (reply to 304771)
...
RootTon: Всем привет, а я ж правильно понимаю что хэш что подчеркнут красным до завершения транзакции всей не получить? прост я смог вывести только хэш что выделен желтым из boc
...
RootTon: принял спасибо @agapak @TrueCarry
...
RootTon: Всем привет, подскажите пожалуйста, можно ли задеплоить смарт как wallet v5 в blueprint? Или blueprint это еще не поддерживает?
...
RootTon: разве? я у них в ридми нашел как указать кастомную версию но можно только v2 и v4 (reply to 304850)
...
RootTon: Немного недопонял, но суть такова, у меня есть смарт который в блюпринте по дефолту делится как v4r2, а мне нужен v5 и вот я хз поддерживает ли блюпринт это или мне щас искать способ задеплоить смарт v5 через что то другое (reply to 304855)
...
RootTon: примечание: у меня последний bp 0.32.1
...
RootTon: есть gql ) он быстрее будет по идее (reply to 304862)

RootTon: да (reply to 304859)

Alexander: Подскажешь какой то пример или где доку почитать именно по транзам ? (reply to 304863)

RootTon: у тон апи же есть gql поддержка вродь

Оскар: тогда решение которое сказали выше, ты указал v5, а надо v5r1 (reply to 304864)

RootTon: а принял спасибо, помогло 🫶 (reply to 304867)

RootTon: ну  задеплоил и оно успешно, ток нюанс такой чисто косметический, поч тут нету contract_type: v5r1 ? в то время как у кошельков обычных такое есть для примера (reply to 304867)

RootTon: ps: и в тонскане пишется мол тип контракта unknown, это на что то может повлиять или тупо визуал ? (reply to 304870)
...
RootTon: Логично однако 😅 (reply to 304872)
...
RootTon: 1) на основе сид фразы конектить чето извращенное но возможно тупо из сид фраз сделать адрес в tonutils есть для этого методы  2) chain walletstateinit и пуб ключ вроде тоже можно у tonutils взять или tonapi чето из этого точно дает помню 3) tonutils (reply to 304983)

RootTon: Примечание: в реале никакой адекватный человек не даст вам свои сиды, так же часто подключение делается именно через tanConnectUI и делается это на стороне клиента/фронта (reply to 304987)
...
RootTon: на то это и примечание ) (reply to 305015)
...
RootTon: Внимательность ;) (reply to 305112)
...
RootTon: Всем привет, есть ли у кого нибудь примерный мок транзакции в стадии выполнения? Мне нужно для личных тестов
...
RootTon: Карта тон чтобы в пятёрочке криптой расплатиться можно было)) идею я подкинул дальше сами) (reply to 305212)
...
RootTon: Если это станет массовым то закон не будет работать ) (reply to 305215)
...
RootTon: Д блииин и то и то будет клево смотреться) приходите вы такой в пятёрочку и оплачиваете чипсы картой с красивым принтом от тона и у вас с вашего тонкипера списываются тонны (reply to 305218)

RootTon: А как оно там дальше это уже решения тех кто решится это реализовывать )
...
RootTon: Д блиииин не порть мою сказку ! (reply to 305230)
...
RootTon: И чисто теоретически кошельки же при переводе тоннов симулируют транзы в том числе и проверяют хватает ли тона на балансе. И по такой логике можно разработать механизм оптимистичной покупки 🤔 (reply to 305230)
...
RootTon: Засчитано 😂 (reply to 305235)

odnokasanie: Это нарушит законы некоторых стран (reply to 305234)

RootTon: А есть кто из тонкипер кто отвечает за вакансии в чате ? Если можно хотел бы в лс пообщаться Ps: не по поводу карты чтобы оплачивать покупки в пятёрочке Pss: я просто лох без LinkedIn и собственного сайта, обычный крестьянин
...
RootTon: А может ссылку для ознакомления? (reply to 305257)
...
Fantastic: Всем привет помогите пожалуйста, я тут новичок и вообще никогда ничего подобного не делал, пытаюсь разобраться. Я хочу запустить свой стейбл но не понимаю как его задеплоить правильно. Я правильно понял то, что я например копирую репозиторий в гит хабе usdt на тоне и меняю название под свой сиейбл и подставляю свой контракт?

Fantastic: ???

Fantastic: Или где-то есть примеры минт/берн контрактов?

Fantastic: Кроме usdt?

Max: Что бы запустить стейбл надо много денег в ликву положить) (reply to 305337)

Fantastic: Я же не говорю что вопрос денег. Вопрос в том как? (reply to 305341)

sd: minter.ton.org, запускайте

Fantastic: Так это будет обычный стандарт жетон (reply to 305343)
...
Fantastic: Минт/  Берн или базовая ликва. Меня интересует выпускать жетон под покупку и сжигать под продажу (reply to 305347)

Fantastic: Купил на 100usdt минтится 100 стейблов грубо говоря с учетом комиссии. И наоборот при продаже тем самым актив обеспечен на 100% но у тона регулятор требует не просто минт/берн но так же фриз опцию

Fantastic: Отсюда и вопрос только в usdt на тоне есть реализация?

Fantastic: Кто-то стейбл писал уже на тоне?
...
Fantastic: Есть примеры других стейблов? Может у кого был опыт? Просто я вижу на тоне только 11 стейблов
...
Fantastic: Алгоритмический рассматривали как временной мерой но на бнб получилось такой сделать (reply to 305354)

&rey: Сжигать — если есть пул, что именно сжигать, то конечно! Если нет — game over (см. историю терры и луны). (reply to 305354)

Fantastic: На тоне:  Выпускаю жетон минтером Подставляю свой бот который управляет курсом  Создаю несколько пулов под АММ для выравнивания   Вы это Алгоритмический называете?
...
RootTon: https://t.me/devs_cis (reply to 305415)

Fasad: Там банят без объяснения причин (reply to 305416)

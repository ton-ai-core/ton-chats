...
hello: tonConnectUI.sendTransaction

hello: te6cckEBAQEAOAAAaw+KfqUAAAAAAAAAADB6EggAsbO5wwO+b62tNRL2YWswtCs8TMtvgNk5gda52S9Kw24QC+vCAfO+QAw=    这个payload参数有什么问题？不能完成支付。

Combot: Combot выдал(а) предупреждение PPOSS (1/3) Reason: 1 reports (reply to 312113)

User: @tonnel напишите Mr Freeman, это создатель tonnel (reply to 312108)

Kon: Немножко не в тему. 😅 но я думал это прикол.

Diva: Ну я написал ему , но что то сомневаюсь что он ответит))) (reply to 312117)

Andrey: Хехе (reply to 312062)

Denis: Ну у вас ещё Кафка и кубер (reply to 312124)

Andrey: Ненавижу кафку, но использую. Мазахизм какой-то (reply to 312126)

/B4ckSl4sh\: грефневую или манную?  (как в тупом анекдоте, сори) (reply to 312127)

Eugene: Сама шутка ок, но канал @AwesomeKafka_ru шутит её своим названием уже восьмой год... (reply to 312128)

Андрей: Привет. Есть свободные кодеры на Tact? Нужен смарт.

Artem: Всем привет. Подскажите пожалуйста как и где запустить смарт контракт, чтобы потом адрес смарт контракта отправить в ton dns с целью создания поддомена?

&rey: Ну, в блокчейне. Через blueprint или ide.ton.org, например, можно. (reply to 312143)

Fragment: Hi, I’m testing a Telegram escrow bot. Can anyone send me 100-200 test TON for testing ?

fruitful-l: Как в контракте реализовать get функцию с балансом контракта? Я пытаюсь получить ее через get_balance, и вернуть через первый элемент кортежа именно TON return(first(get_balance())); Но это судя по ошибке неверный подход

fruitful-l: Такое решение выглядит немного странным, оно оптимальное? (int) getBalance() method_id {     [int ton_balance, _] = get_balance();     return(ton_balance); }

/B4ckSl4sh\: Зачем для этого гетметод? В любом api который вы используете будет способ получения баланса. Я даже не уверен, что код выше будет работать в силу особенностей запуска гетметодов (reply to 312163)

fruitful-l: Балуюсь с контрактами web IDE, изучаю funC, деплою в сендбокс и нужна просто затычка, чтобы я мог иногда поглядывать на баланс, не сильно при этом заморачиваясь (reply to 312164)

Sutax: У кого есть сендер транзакций с сообщением? Могу заказать у вас рассылку

fruitful-l: При деплое в тестовую сеть работает вполне нормально, чисто из интереса, из-за каких особенностей запуска гет методов он мог бы не работать? (reply to 312164)

/B4ckSl4sh\: Теоретически апи провайдер может забыть проставить баланс контракта при запуске гетметода на нём, хотя может и у всех есть. В общем я бы на надёжность этого способа сильно не рассчитывал (reply to 312167)

fruitful-l: Понял, спасибо! (reply to 312169)

— 2025-07-03 —

Just1k $BC: Test environment For test environment you can use this parameters:  BCL Master address - EQBSMwczMFUb789uqNvebKBvemkRaAQJdzTFq6565Ef9rW2k  TonFun api address - https://test-indexer-3a4wy.ondigitalocean.app/api/v1   Где брать значения для майнет ?

Сергей: Привет А может кто-то рассказать что за жеттон такой proxy Ton? Он существует только внутри свапа?

sd: Просто обёртка стонфая чтобы все свопы шли по одной схеме (reply to 312180)

Сергей: А в теории пользователь может их как то закупить? Когда разбираю по jetton events появление и отправку этих жеттонов, они как будто из ниоткуда появляются Типо адрес просто резко взял и отправил 1pTon, хотя до этого его не получал на свой адрес (reply to 312182)

Denis: Адрес получил тоны

Сергей: И вот этот wrap он не отображается никак в ивентах?

Denis: https://github.com/ton-blockchain/TEPs/pull/161/files (reply to 312185)

Сергей: Спасибо (reply to 312186)

Grigorii: Всем привет, хочу парсить инфу с контракта о каких то транзакциях + доп данные из контракта  Нормальный ли кейс: выкидывать из контракта ext_out_msg со всей инфой которая мне нужна?

Егор: Приветствую, есть ли здесь человек, который может доработать смарт контракт?

User: Да, контракт жетона blum так пишет логи свои (reply to 312191)

Nick Seenai: Всем привет Буду благодарен, если поделитесь чатами / каналами по поиску исполнителей на дизайн / разработку

User: https://t.me/addlist/1eHg7chOHchlNTM0  Папка со всеми чатами разработки в телеграмм и TON  Также вы можете сформулировать техническое ТЗ и попробовать поискать разработчиков здесь, но будьте осторожны — мошенников, которые убегают с предоплатой или тех кто имитирует бурную деятельность с помощью ИИ и после не выдает результат тут море. Будьте бдительны, не кидайте предоплат и проверяйте компетентность программиста лично! (reply to 312198)

Nick Seenai: Большое спасибо (reply to 312200)

Вика: Спасибо посмотрела, но там используют ton-core и ton, а в том же blueprint для такта враперы на @ton/core и @ton, и опытным путем у них есть разночтения и вроде как её полная совместимость  Может быть есть какие-то более свежие ресурсы по созданию фронта? (reply to 311908)

Oleg: @ton/core и @ton это современные версии

Oleg: курсы слегка устарели, но все еще при небольшой доработке напильником полезны

Вика: Они предлагают ton access использовать и там все примеры на старых либах, а на новых им пользоваться можно? (reply to 312207)

Oleg: Я, когда проходил этот курс больше года назад, просто позаменял либы на новые и все заработало (reply to 312208)

Вика: Ладно сейчас попробую (reply to 312209)

Just1k $BC: Подскажите по ответу. С Тон Фан кто работает ? (reply to 312172)

Grigorii: А как через консоль запустить какой то конкретный тест? blueprint типо там же должен быть какой то флаг: npx blueprint run --...

Nick: npm test — tests/MyTest.spec.ts

This photo: Добрый день, коллеги, нахожусь сейчас в поиске работы в данной сфере. Написал чистосердечное письмо-резюме для рассмотрения своей кандидатуры теми, кто имеет вакансии.

web-assist 🧀: Это конечно круто, но такое никто читать не станет. Напиши нормальное резюме или CV (reply to 312231)

This photo: Спасибо большое за фидбэк, есть также резюме в более привычном формате. (reply to 312232)

vv: Советую не открывать рандомные пдф файлы в тг чатах, если не хотите потерять что-либо)

web-assist 🧀: Все что извне открываю в изоляции, был уже плачевный опыт (reply to 312234)

fruitful-l: Добрый день. Появился вопрос по логике написания контракта. Есть такой код, контракт деплоится только с initial адресом владельца (пока что). Возможно у меня уже закипели мозги немного, но не совсем понимаю, что делать с Cell Underflow, когда при деплое контракт попытается считать не только адрес, но и опкод, для дальнейшей логики. Стоит ли просто добавить специальный оп-код для такого, или есть идеи как это чище реализовать?  () load_data() impure {     slice ds = get_data().begin_parse();     state_init = false;     storage::author_address = ds~load_msg_addr();               if (slice_bits(ds) > 0){                  int state_init = true;         goal = ds~load_coins();      } }  () save_data() impure {     set_data(begin_cell().store_slice(storage::author_address).store_coins(goal).end_cell()); }  () recv_internal(int my_balance, int msg_value, cell in_msg_full, slice in_msg_body) impure {     slice mb = in_msg_full.begin_parse();     int fg = mb~load_uint(4);     if (fg == 1) {         return();     }        slice sender = mb~load_msg_addr();     load_data();          if(~ state_init) {         goal = in_msg_body~load_coins();         donators = new_dict();         in_msg_body.end_parse();          raw_reserve(fee::initilization, 4);         send_raw_message(begin_cell()             .store_uint(0x10, 6)             .store_slice(sender)             .store_coins(0)             .store_uint(0, 1 + 4 + 4 + 64 + 32 + 1 + 1)             .end_cell()         , 128);                  save_data();         return();     }          int op = in_msg_body~load_uint(32);

fruitful-l: В целом ошибка не мешает самому деплою, но я так понимаю игнорировать будет не самым красивым решением

Oleg: Обычно контракт деплоят в 1 этап, чтобы в этой же транзакции он развернулся и принимал сообщения. Конечно же, надо сделать полноценную обработку опкода (reply to 312237)

fruitful-l: Понятно, спасибо! (reply to 312239)

Grigorii: Про такую штуку знаю, но она запускает сразу все тесты в файле. А как запустить отдельный? webstorm может так запускать, но я не понимаю как (reply to 312227)

Nick: -t “mysuite mytestcase”

fruitful-l: Не совсем понимаю, ошибка на моей стороне и неправильных данных? Деплой контракта через blueprint, в web Ide тоже не дает задеплоить   [TON_CONNECT_SDK] Wallet message received: { error: { code: 1, message: 'ServerError' }, id: '3' } BadRequestError: [TON_CONNECT_SDK_ERROR] BadRequestError: Request to the wallet contains errors. ServerError

Sergey: О, такое ощущение, что проблемы ton connect

Sergey: Я ещё попробовал у себя демку из репы ton connect, там тоже траблы:(

User: Без кода сложно сказать, но проверьте передаете-ли вы поле validUntil (reply to 312246)

fruitful-l: Не совсем понимаю откуда это поле. Не хочу полностью код сюда кидать, много места занимает, но я чуть выше скинул всё, что вызывает эту реакцию (reply to 312249)

Denis: Россия? Впн в помощь

fruitful-l: Впн стоит и на телефоне с кошельком, и на ноутбуке с контрактом, не помогает (reply to 312251)

User: Я посмотрел код, он на func, но ошибка которую вы скинули заключается в tonconnect. Может быть вы при формировании транзакции вы забываете указать поле validUntil в Tonconnect (reply to 312250)

fruitful-l: Какой код прислать лучше? Как я понимаю скрипт деплоя? (reply to 312253)

User: Да, на ts (reply to 312256)

fruitful-l: export async function run(provider: NetworkProvider) {      const donateContract = provider.open(DonateContract.createFromConfig({author_address: provider.sender().address!}, await compile('DonateContract')));     await donateContract.sendDeploy(provider.sender(), toNano('0.05'));     await provider.waitForDeploy(donateContract.address);

User: Ничего подозрительного не наблюдаю. Скиньте пожалуйста еще исходники sendDeploy (reply to 312258)

fruitful-l: конфиг, wrapper? (reply to 312259)

VAN: Привет! Нашла мошеннический TON DNS-домен. Могу ли отправить сюда? Используется для обмана под видом Binance и бейбит. Прошу проверить и добавить в чёрный список, если возможно. Могу отправить скриншоты переписки

&rey: Вы же помните, что разделегировать TON DNS можно только полным голосованием валидаторов? (reply to 312264)

User: Конкретную функцию внутри объекта DonateContract — sendDeploy (reply to 312263)

fruitful-l: Как я понимаю проблема была с TC. Вроде и жаловались в чате на сбои. Ничего не менял, попробовал еще раз и контракт успешно деплоится. Вот функция на всякий случай async sendDeploy(provider: ContractProvider, via: Sender, value: bigint) {         await provider.internal(via, {             value,             sendMode: SendMode.PAY_GAS_SEPARATELY,             body: beginCell().endCell(),         });     } (reply to 312266)

Надежда: Хочу пожаловаться и внести в черный список 2 домена  Этим доменом пользуются мошенники - они выдают его за официальный адрес Binance и предлагают переводы TON под видом обмена на USDT.  Прошу проверить и, если возможно, добавить этот домен и связанный адрес в чёрный список или предупредить пользователей в кошельках Tonkeeper/Tonhub, чтобы никто не попался. Бинанс и bybit уже предупредила Могу отправить?

User: Понял. Ни где в ваших файлах нет явных логических ошибок, так что с учетом того, что ошибка сама себя вылечила, то скорее всего дело в вашей геолокации и в том, что tonconnect не смог подтянуть какие-то данные, как об этом писал выше Суботин  Я ловил похожую ошибку единожды, но тогда оказалось что у меня лагал интернет (reply to 312267)

&rey: Таких доменов, вообще говоря, больше. Но вы же помните, что разделегировать TON DNS можно только полным голосованием валидаторов? (reply to 312270)

Надежда: Да, помню (reply to 312274)

— 2025-07-04 —

Combot: Привет, CiaoAI William1! Прочти правила и нажми кнопку ниже.

Oleg: Тогда нейронка все правильно вам говорит (reply to 312283)

Никита: Но если я зачеканю, это будет не то? Я видел в кошелке телеги USDT и мне казалось что можно как то с ним взаимодействовать

web-assist 🧀: Зависит от того, что подразумевается под управлением. Если просто менеджить их и рассылать туда-сюда - можно (reply to 312283)

Никита: Да, замораживать, рассылать (reply to 312286)

Никита: А как? Где я могу бы про это почитать? (reply to 312286)

web-assist 🧀: https://docs.ton.org/v3/documentation/ton-documentation (reply to 312288)

potato: А ты раньше с бч работал хоть как-то? Что значит "замораживать" (reply to 312288)

Никита: Нет( То и значит, оставлять на балансе смарт контракта до выполнения условий (reply to 312290)

Никита: Так нельзя?

web-assist 🧀: Можно, просто не трогаешь их и они никуда не денутся)

web-assist 🧀: Если надо лекции или онбординг в бч - пиши в лс) На каком ЯП ты работаешь?

Vladimir: Вы для доски объявлений? (Почитал в канале). Лучше сразу описывать бизнес задачу. (reply to 312291)

Никита: угуу (reply to 312299)

Vladimir: Принимаете на свой кошелёк USDT, а как только покупатель подтверждает получение — отправляете продавцу. (reply to 312300)

&rey: А вы уже решили, откуда брать реальную информацию, дошло там что-то или нет? Пока что у вас единственным источником служит купивший. (reply to 312300)

Китикет: Тоже об этом подумал, как поступать в ситуации если покупатель усиленно утверждает что ничего не пришло, а продавец что посылку забрали) (reply to 312302)

Никита: одна из идей брать инфу по статусам из api доставки, но в таком случае нужно как то конвертить крипту <—> фиат (reply to 312302)

Никита: а как в таком случае поступает p2p биржа? по типу binance (reply to 312303)

Китикет: Можно наверное сделать чтобы перед сделкой покупатель указывал в системе адрес, и продавец строго по нему отправлял каким-нибудь сервисом в котором можно зайти по публичной ссылке и посмотреть забрали посылку или нет

Китикет: В случае если там посылку забрали, считаем что покупатель ее получил и можно отдать деньги продавцу Если протестует и система говорит что забрал - значит хуйней страдает и надо отдавать деньги продавцу все равно

Китикет: Ну тут хз, не в курсе) (reply to 312305)

&rey: Фиат традиционно не может застрять нигде, и можно смотреть на фотографии чеков. Посылку сколько ещё досматривать будут. (reply to 312305)

Китикет: разве что надо чтобы покупатель мог посмотреть товар перед тем как забрать короче процесс очень запарный конечно и все кейсы покрыть невозможно (reply to 312306)

Китикет: а то продавец может со своей стороны выкинуть фигню и отправить что-то другое, и покупателю нужно будет изначально видос записывать как забирает и открывает (и это тоже можно подделать)

&rey: Кажется, стоило бы добавить человека-свидетеля, кто бы мог присутствовать при получении и кто записал бы все претензии.

&rey: Ну и если совсем бред будут везти, добавить ключ платформы, чтобы она могла отменить заказ (если кто-то подтвердит).  Ура, мультисиг два из четырёх!

Dimer: Подскажите мне новичку, как верно получить актуальные данные наград за стейкинг? Насколько я понял, это можно сделать с помощью транзакций в блокчейн с помощью метода из смартконтракта типа update_reward. Но, что-то я сомневаюсь, что так действительно получают данные по стейкингу. Возможно, такие данные можно получить через апишку? Сам по себе стйкинг парсится, показывается твл, периоды, комиссии. Но награды - нет. Как же получить актуальную награду в реальном времени за стейкинг на фронте?

L30n1d: а что за стейкинг? (reply to 312329)

HORNY☦️ ❘ Скупщик нфт: а

Max: Похожий проект буквально неделю-две назад закрылся из-за отсутствия интереса со стороны сообщества  Не страшно? (reply to 312348)

aaaaaa: неа, ведь аудитории у нас достаточно и мы не сидим просто так) (reply to 312349)

aaaaaa: сам можешь понаблюдать на наши объемы на ивентах, которые закрылись

Max: это да, поживее выглядит  только кто тот гений, что придумал такой формат дат? Аж глаз коробит  https://www.iso.org/iso-8601-date-and-time-format.html (reply to 312351)

Combot: Combot выдал(а) предупреждение aaaaaa (1/3) Reason: 1 reports (reply to 312348)

Andy: Очередные ставочки. Этот тот проект, который рекламирует какой-то православный активист? (reply to 312350)

aaaaaa: а православный активист это кто?)

Andy: Я не помню его имя. Есть похожий проект, там в команде как-то чел, который у себя про себя в CV так и пишет

Andy: Вы предлагаете заключать пари. Лицензия есть на эту деятельность?

Andy: Ясно, быстро слился. Очередной скам, кинул репорт

🏆: каким образом мокнуть телеграм мини апп, чтобы можно было тестировать будто бы из "телеграм браузера"? нужно посмотреть запросы к апи.

Max: Не в этот чат  Но, отвечая на ваш вопрос, самое простое это использовать web.telegram.org (reply to 312363)

Dimer: Пользователь стейкает свои жетоны. После этого он получает ежесекундно награды. И вот теперь вопрос, как эти награды проверить? В смартконтракте есть метод update reward, но он работает как отдельная транзакция к кошельку. Вот мне интересно,  есть ли альтернатива, как узнать свою награду? Может есть апи запрос какой к кошельку? (reply to 312341)

Oleg: Читаешь время стейкинга, объем котлеты и все. Текущее время известно (reply to 312368)

Роман: всем привет! а подскажите пожалуйста, как правильно добавить в контракт логику замены кода и данных.  я пришлю на контракт специальный op_code и добавлю 2 ссылки на новый code и на новый data.   вопрос в том, как это правильно сделать, accept_message нужно делать до изменения кода и данных?  И сразу нужно set_code делать, а затем set_data? и потом return (); правильно будет?   а ещё вопрос что мой seqno потеряется (так надо))) но для защиты от дублирования, можно ли условно сравнить два хэша кода новый и существующий, чтобы не произошла замена если они совпадают?   Был бы очень благодарен приведенному коду или ссылки на пример) answers.ton.org не грузит😔

Oleg: https://tonviewer.com/EQCxE6mUtQJKFnGfaROTKOt1lZbDiiX1kCixRv7Nw2Id_sDs?section=code  В jetton-minter.fc есть пример обработки op::upgrade (reply to 312378)

fruitful-l: Есть ли где-то референс задания on-chain метадаты для NFT и Jetton?

— 2025-07-05 —

User: https://github.com/ton-blockchain/minter-contract/tree/main#jetton-metadata-field-best-practices  Посмотрите тут. Все важные детали расписаны. Если будет что-то не понятно после, можете спрашивать тут или у нейронки(последние модели достаточно хорошо анализируют информацию и способны ее пересказать в простом виде)  Если вам нужны примеры в виде уже сминченных жетонов/nft: https://tonscan.org/jetton/EQCdpz6QhJtDtm2s9-krV2ygl45Pwl-KJJCV1-XrP-Xuuxoq — PUNK/On chain  https://tonscan.org/jetton/EQCxE6mUtQJKFnGfaROTKOt1lZbDiiX1kCixRv7Nw2Id_sDs — USDT/external  И вот еще по поиску в чате нашел пример кода на ts: https://t.me/tondev/272190

GG: Всем привет. У меня одного проблема с TON?  Из других сетей не бриджится, с бирж закрыт вывод в сети

Nikolas: Что с тон сайтом в телеги. Через тон прокси отрывается. В телеги ни как с впн  тоже.

fruitful-l: Большое спасибо (reply to 312392)

Randall: У меня проблемы с нодами. Началось всё 12 часов назад, когда балансы некоторых аккаунтов неправильно стало показывать. Сейчас экстерналы не принимаются с ошибкой "Cannot run message on account", exit code 33. И лайтсервера запросы не обрабатывают, выдают ошибки "Failed to unpack Account". Как будто БД похерилась. (reply to 312396)

Randall: Кто-нибудь знает, как пофиксить?

🐼 Sild: Звучит так будто надо обновить ноду  На неделе как-раз патч с новым стейтом раскатился

Василий: 33 это некорректный seqno (reply to 312410)

Randall: это после обновления и началось)) (reply to 312412)

Sutax: Жалею что не обратил внимания на эту жалобу от 21 февраля (reply to 291662)

⛰️ Roman ⛰️: Комиссия за сообщение в контракт зависит от курса TON？

Дмитрий: Ждем от вас пруфы. Не разу не видел чтобы менеджер кого-то кидал, насколько я знаю по чату и логу вам не понравилось что ночью упал сервер и скрипт несколько часов не работал, а в остальном все хорошо и это вы на ходу меняите условия и решили спустя сутки без 4 часов работы скрипта слиться от оплаты (reply to 312415)

Дмитрий: UP. Заметил что про комиссию вас не устроило что блокчейн брал за транзакции ваши скам

potato: Не первый раз жалуются на @scalien и не только в этом чате) Остерегитесь прежде чем с ним сотрудничать

Дмитрий: Опять затычка твоя (reply to 312421)

Дмитрий: Жалоба без обоснований,  выше ответ неё и это он кинул по факту нас.

wagelina {B I O}: насколько может быть правдой?

wagelina {B I O}: ну или скажите пожалуйста, где искать ответы по поводу апдейта

Nick: В чате с валидаторами, официальном репозитории или канале тонстатус.  Учтите, если играть в "казино", скорее всего вы потеряете свои средства. (reply to 312432)

⛰️ Roman ⛰️: если баланс контракта будет потрачен полностью и он будет "отключен", то что бы его снова включить нужно отправить ему немного TON или еще нужно будет отправить все его данные, которые были в c4 на момент, когда он "отключился"?

fruitful-l: Вроде если он не заморожен, storage остается таким же (reply to 312440)

fruitful-l: Подскажите, есть ли возможность как-то скипнуть coins в сообщении? skip_bits явно не работает из-за разного размера

&rey: Ну, ~load_coins() и выбросить результат. (reply to 312443)

&rey: -0.1 тон (заморожен) — отправить State (c4, код, библиотеки и т.п.) -1.0 тон — уже не важно, он удалился. (reply to 312440)

fruitful-l: Точно, почему-то забыл что так можно сделать 🥶 Спасибо (reply to 312445)

Combot: Combot выдал(а) предупреждение boscowr (1/3) Reason: 1 reports (reply to 312449)

— 2025-07-06 —

🕵️: Кто-нибудь может подсказать есть ли парсер метаданных универсальный (для Jetton, Nft и семичейн, ончейн, оффчейн)

User: Советую воспользоваться централизированной апишкой вроде tonapi или toncenter, если проект позволяет (reply to 312469)

🕵️: А варианта с библиотекой и скриптом на JS/TS нет? Универсального

User: Ничего не вспомню, так что скорее всего готовых решений нет. Может меня подправит кто-нибудь чуть позже. Но вы можете написать свое решение чтоб получать все эти данные самостоятельно (reply to 312471)

🕵️: У меня есть решение, оно в принципе работает со всеми жетонами, но у тезера не подтягивает decimals (reply to 312472)

User: https://github.com/ton-blockchain/minter-contract/tree/main#jetton-metadata-field-best-practices  Это поле опционально. Но если взять конкретно USDT, то у него метадата смикшена, потому-что информация о decimals находится внутри контракта, то есть ончейн, все остальное оффчейн (reply to 312473)

🕵️: У него идет флаг 0x00 дальше decimals и потом урл с метаданными (reply to 312474)

BIN: Приветствую, есть где скрипт на создание v4r2? А то v5r1 совсем какая-то ебота ничего с ним не работает, получаю через WalletV5R1.from... – другие адреса

BIN: Или как в мтв открыть в4р2
...
Muhammad: Привет! Я хочу подать заявку на грант TON Foundation.  Я разработал Telegram MiniApp под названием «TON Guard Bot». Он позволяет: – Подключить кошелёк через TON Connect   – Смотреть баланс и NFT   – Проверять адреса на скам через TonAPI   – Получать алерты на whale-транзакции    У меня готов MVP, опубликован репозиторий на GitHub и есть демо-видео.  Сайт https://grants.ton.org у меня не открывается (возможно, из-за региона).   Подскажите, пожалуйста, как я могу подать грант вручную или куда отправить заявку?  Мой GitHub: https://github.com/Maga-bit-sudo/ton-guard-bot    Спасибо заранее!

Muhammad: Могу ли я подать заявку для роста Тона и улучшить своего бота за выданное мною средства ?

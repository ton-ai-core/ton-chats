...
Dmitry: Ну в нфт вшит не сам mp3 файл (а то дорого будет хранить), а ссылка на него. А куда ссылка ведет? На какой-нибудь публичный ipfs/cdn/storage (reply to 321312)

Сергей: Привет, подскажите где подключить для бота API для виртуальных карт visa?

Aziz: В кнопке, но вопрос не про ТОН (reply to 321329)

Lunar shop manager: Через какую библиотеку на пайоне лучше делать авто списание средств при покупке на fragment?

RR: Привет, я на днях в тма билд скинул мвп своего проекта, можешь там выпустить) (reply to 321312)

RR: Кстати, подскажите, пожалуйста , есть ли возможность получить тестовые usdt как тоны?

RR: нфт твой, сминченный в приложении, автоматом появится на гетгемс (reply to 321374)

Oleg: Просто сминти их, они все равно у каждого свои (reply to 321375)

RR: Можно чуть-чуть подробнее, я не шибко разбираюсь в этом пока (reply to 321377)

Oleg: Вот тут например https://minter.ton.org/?testnet=true Вбиваешь данные про твой будущий токен, сколько децималей, сколько их тебе надо и готово (reply to 321378)

RR: Благодарю! (reply to 321379)

Not: Закинь в лс если не сложно как найти то (reply to 321374)

RR: Закинул (reply to 321381)

Lunar shop manager: Через какую библиотеку на пайоне лучше делать авто списание средств при покупке на fragment?

Леша: На питоне, вроде нет конкрктной библиотеки для покупки звезд через фрагмент, но я на расте писал давно, можете переписать на питон https://github.com/malarry/fragment-api-rust (reply to 321383)

Mirka: Если что, у USDT 6 знаков после запятой(у других токенов в основном 9) (reply to 321375)

— 2025-09-13 —

X1ag: подскажите пожалуйста а tonapi embeded можно в проде использовать? Prod-ready оно? просто совсем недавно узнал про это  https://docs.tonconsole.com/payment-processing/tonapi-embed

Denis: Можно. Продакшен реди оно. На нём одна биржа крупная работала. Только оно клозед сорс и платно через договор

Denis: Хотя мы тут подумываем отдать в опенсорс

X1ag: Понял, спасибо

&rey: А оно нормально обрабатывает, если кинуть 255 трансферов на депозитный адрес из одной транзакции? (reply to 321420)

Denis: конечно.

Denis: аж обидно стало

&rey: Да я уж испугался, увидев описание /account/{address}/history/{hash})

X1ag: А сколько времени тонвивер хранит в кеше старую метадату? Т.е. я задеплоил с одной метадатой, потом поменял этот файл и через сколько времени новая метадата будет на тонвивере?

Denis: Там сложная конструкция. От суток до 7

X1ag: спасибо (reply to 321432)

Oleg: А это фича, что Tonkeeper браузерный позволяет увидеть баланс кошелька, не вводя пароля? Вроде нигде такого вопиющего безобразия больше нет

&rey: Зависит, скорее всего, от модели угрозы. Кто-то подсмотрит экран из-за спины? (reply to 321434)

Denis: Да и мобильный. Там в настройках включается для параноиков (reply to 321434)

Oleg: Нет, я про "маски шоу". Когда твой ноут попадает в руки органов (reply to 321436)

&rey: Тогда нельзя и адрес для пополнения показывать, потому что там всё из блокчейна будет видно. (reply to 321439)

Denis: Да и история открытий вкладок тонвьювера несколько палит ваши балансы (reply to 321440)

Oleg: Как по мне, логично просто всегда требовать пароль при открытии, если время вышло

Denis: От такой угрозы ещё и пароль должен быть символов 14 минимум с цифрами и раскладкой иначе толку не будет. Заставлять пользователя по-умолчанию такое вводить каждый раз когда он пришёл график стоимости тона глянуть это садизм. Включите в настройках если серьёзно расматриваете такой вектор угроз

sd: Главное чтоб это было в настройках, у меня на деске стоит пароль и я так и не понял когда его надо вводить, похоже что никогда (reply to 321443)

sd: При транзакциях не просит спустя время не просит кнопки лока паролем тоже нет, чисто косметический

Oleg: При отправке транзы точно надо, больше никаких настроек в секьюрити не нашел. Только "lock screen", но оно походу ни на что не влияет (reply to 321444)

sd: У меня стоит и не просит (reply to 321447)

sd: Посмотрю потом ещё раз, может что-то ещё включить надо

Denis: Звучит несколько сомнительно. Я им каждый день пользуюсь и у меня просит (reply to 321448)

sd: Я тоже))) и даже писал об этом другому человеку из тонкипера

Denis: А если зайти в настройки то можно включить режим для параноиков

Denis: Будет так

&rey: Ну, 12 слов из того же алфавита 2048 — это 132 бита. Появляется такая модель угрозы, что часть слов известны, часть нет (бумажка порвалась, уголок бумажки сфотографировали, ...), и тогда перебор уже становится в теории возможным. PBKDF немного улучшает ситуацию, это да.

Denis: Переписка выходит немного шизофреническая когда другие не видят сообщения удаленые (reply to 321458)

Dmitry: Зато интересно читать, простор для фантазии. «Введите пассфразу от адреса чтобы разблокировать кошелек» - мечта фишеров 😁

Arsen: Всем привет! Кто-нибудь может подсказать, пожалуйста, как мне получить статус транзакции на бекенде? С фронтенда приходит инфа о новой транзакции с полем "boc", не могу никак понять, как получить статус этой транзакции через tonutils на беке

Dmitry: https://gist.github.com/mr-tron/a4b3e8eff3a982e3426f6397eb9a2450 Вам вернулся boc of external message body (reply to 321466)

Arsen: Благодарю! (reply to 321467)

— 2025-09-14 —

фф: Ребят. У меня есть cell: cellBoc = te6ccsEBAwEAngBqECQCy5TrfxqvZ3E39ZM/AnecvmiA8HxYHYEeR+95ghbqmpO5pEoDALqhG6V691bqZVnohSllrkCTI2dXf3EY..   const transfer = contract.createTransfer({       seqno,       sendMode: SendMode.PAY_GAS_SEPARATELY,       secretKey: keypair.secretKey,       messages: [         internal({           to: "0:10226fe1c8e4028719b46397453abf6898603051e7d80a8059914dfc5563d0cf",           value: "10.00",            bounce: true,           body         })       ]     }); как отправить его? подскажите пожалуйста   const cellBoc = "te6ccsEBAwEAng...     const body = Cell.fromBase64(cellBoc); Это неправильно же?

Combot: Combot выдал(а) предупреждение Марьяна (1/3) Reason: 1 reports (reply to 321487)

Scilef🦉: Добрый день, уважаемые разработчики. Подскажите-помогите: не могу понять, что я делаю не так. Есть у меня две строчки: save_data(owner_address, beneficiary_address, nft_item_code, tiers, current_item_index, current_tier_id, is_busy); throw(100 + current_tier_id); Compute phase падает на save_data с exitCode 7 (в тестах через sandBox). Если я меняю их местами - выдаёт 101 (значение из throw). То есть, ошибка именно в save_data. Но я уже подставил туда те же значения, что кодом выше беру из load_data - всё равно падает, хоть ты тресни. В чём может быть причина?

ytiruces: Добрый день. У меня почему-то метод wallet.contract.getSeqno() периодически рандомно выдает не актуальный seqno. По факту например 1758, а метод возвращает 1680, потом несколько запусков все норм, а потом снова 1680, причем в разных точках вызова. Было такое у кого-то?

⛰️ Roman ⛰️: Как я помню при загрузке данных откуда то к ним не автоматически присваивается тип, вам нужно принудительно указать тип. Как то так (reply to 321500)

Scilef🦉: Там всё интереснее Сделал дамп, в транзакции до значение переменной в сторейдже 1, я его сохраняю  В следующей транзакции я его считываю, кидаю в дамп - в логах 2500000 ._. (reply to 321502)

&rey: К какому апи обращаетесь? Верхний мастерчейн-блок там всегда возрастает, или запрос может уйти на недосинхронизированный лс? (reply to 321501)

ytiruces: https://testnet.toncenter.com (reply to 321507)

&rey: Тестнет – это, скажем так, тестнет. К нему интерфейсы вечно падают. (reply to 321508)

ytiruces: Воспринимать это как нормальное поведение? (reply to 321509)

&rey: Скорее как случайную сетевую ошибку – ну, провалился запрос, надо повторить. (reply to 321510)

&rey: Да, вы же помните, что с v4/v3 можно отправлять по 4 сообщения за раз, а с v5 все 255?

ytiruces: Пока не очень, учимся (reply to 321512)

Stas: Сап. Что может быть не так?  const seqno = await contract.getSeqno() const transfer = await contract.createTransfer({   seqno,   secretKey: keyPair.secretKey,   sendMode: 3,   messages: [internal({     value: toNano('0.01'),     to: 'UQBSQwoAA11_WR1851Tptx0jv5M_QJFlOpTUM6a5N_Ia0MN9',     body: 'Test'   })] })  await sleep(5000)  await contract.sendTransfer(transfer)  Ошбика:  /@ton/ton/dist/wallets/v5r1/WalletContractV5R1.js:130         const actions = args.messages.map(message => ({ type: 'sendMsg', mode: args.sendMode, outMsg: message }));                                         TypeError: Cannot read properties of undefined (reading 'map')  UPD: await contract.sendTransfer(transfer) => await contract.send(transfer)

фф:     await client.sendExternalMessage(wallet, transfer)  я вот так отправляю транзу. все работает     const wallet = WalletContractV5R1.create({ workchain: 0, publicKey: keypair.publicKey }); (reply to 321514)

ytiruces: Я отправляю нфт на свой смарт контракт. Можно ли прочитать метадату нфт, его атрибуты, в этом контракте и использовать в его логике? Как?

NFT: Прочитать метаданные NFT, такие как его атрибуты, в смарт-контракте можно, но это нетривиальная задача, поскольку метаданные хранятся вне блокчейна. Смарт-контракт имеет доступ только к данным, хранящимся непосредственно в блокчейне. Метаданные NFT обычно представляют собой JSON-файл, который хранится на децентрализованных файловых системах, таких как IPFS или Arweave, или на централизованных серверах. В блокчейне хранится только URI (Uniform Resource Identifier), который указывает на этот JSON-файл. (reply to 321521)

🐼 Sild: это, ну, не совсем правда.  в метадате может храниться дикт, в котором может быть навалено все что угодно, в том числе - url для внешних данных  на практике да, там часто урл по которому надо сходить  ну и ссылочка для консистентности https://docs.ton.org/v3/guidelines/dapps/asset-processing/nft-processing/metadata-parsing#parsing-metadata (reply to 321522)

ytiruces: Я это понимаю. Просто если я использую свой код нфт или сейл контракта, в которых я расширяю хранилище для хранения атрибутов или меняю логику отправляемых сообщений, то на getgems такие контракты уже не работают должным образом. А если я оставляю ванильные гетджемовские контракты, то я не могу прочитать метадату при их отправке на свой смарт контрак со своей бизнес логикой. Замкнутый круг получается (reply to 321522)

NFT: Дилемма) можно использовать оракулы или создать смарт-контракт посредник"прокси контракт" (reply to 321524)

NFT: Лс (reply to 321524)

Combot: Combot выдал(а) предупреждение Яна (1/3) Reason: 1 reports (reply to 321548)

Grigorii: Всем привет, а есть готовые решения для вычисления натурального логорифма и вычисления exp на func/tolk?

&rey: А как входное дробное число сложено в int257? (reply to 321553)

Grigorii: А если домножать на 2**128 и примерно считать? (reply to 321554)

&rey: fixed point [sign+128.128], понял. (reply to 321555)

Tim: Я бы поискал в stonfi контрактах для старта (reply to 321553)

Petr: Может тут есть что-то похожее https://github.com/ston-fi/funcbox (reply to 321553)

— 2025-09-15 —

Egor: а какой браузер используется в тонкипере, как посмотреть версию?

Tim: системный! (reply to 321578)

Egor: типа тот что по умолчанию выбран? (reply to 321579)

Tim: нет, тот что системный webview

D.K: Всем привет. Может кто знает, есть ли смарт контракт, который блокнет все токены (не только тон) , которые на него переведут , на указанный при деплое период? И что бы изменить его нельзя было

&rey: Ну, например, контракт () main() { } вам подойдёт? (reply to 321590)

Vlad: Пытаюст поднять в Docker compose mylocalton, при up ловлю ошибку:  ✘ lite-server Error    Head "https://ghcr.io/v2/neodix42/mylocalton-lite-server/manifests/latest": denied  Как так?

Dmitry: Нет, он USDT от governance не убережет :) (reply to 321592)

&rey: А это сложнее, надо по непредсказуемой цепочке адресов гнать. (reply to 321595)

Seitaro: ну или обобществить: вложить например в пул usdt-OWN_TOKEN и хранить на контракте-локе LP токены. Чтобы заблочить usdt тогда придется блочить роутер/волт декса. (reply to 321596)

Tim: Про вестинг ведь вопрос а не сжигание (reply to 321592)

Tim: А ты залогинился в гитхаб регистри? (reply to 321593)

Seitaro: контракт который деплоится в vesting.ton.org, если его использовать без собственно вестинга, а только с однократным разлоком, вполне подходит: он не разрешит выводить тоны и не разрешить отправлять никакие сложные сообщения (включая переводы жетонов), а после разлока позволяет отправлять любые сообщения куда угодно. Проблема в том, что интерфейс vesting.ton.org не позволяет отправлять переводы жетонов, придется дапп под себя делать. (reply to 321590)

D.K: Спасибо (reply to 321600)

Илья: Здравствуйте, подскажите написал вместе с Чат ГПТ5 скрипт который бы автоматически делал выводы из Тонкипера после получения туда депозита. Вроде скрипт норм, но ошибка 429. И ее никак не можем убрать. Купил платный API ключ по совету робота, он не помогает мне, ошибка снова. Кто знает в чем суть?

Ivan: похоже что слишком много запросов делаете (reply to 321646)

Илья: Ну я вот сделал ОДИН всего лишь депозит, скрипт должен автоматически вывести, и не может ошибка 429. Всего лишь один вывод не может даже. Купил платный API где 100 запросов в секунду якобы (reply to 321648)

Mirka: Код будет или продолжим гадать? (reply to 321650)

Илья: Щас сфоткаю, я не шарю нифига там (reply to 321651)

Ivan: Надо код смотреть, это же чат гпт может он пытается тысячу параллельных запросов сделать а может он вообще не использует платный апи (reply to 321650)

Ivan: нифига себе

Vladimir: Фотография, еще и в обычном блокноте.

Илья: Я не шарю где открыть и как. Чат гпт сказал в блокнот вставлять

Илья: Может кто-то сказать что не так с кодом?

Mirka: А ты можешь просто его скопировать и отправить? (reply to 321663)

Tim: Ну во первых он в блокноте

Илья: Ща... (reply to 321664)

sd: Хех ты что ли сид фразу по чатам рассылать хочешь (reply to 321646)

Mirka: Оберни в ```, чтобы красиво выглядел (reply to 321667)

Илья: М? (reply to 321668)

Илья: Это как? (reply to 321669)

sd: Ну зачем тебе выводить сразу после получения депозита то, если это кошелёк проекта раз в день выведи и все

Tim: Сколько будет 2+2? Напиши рецепт борща (reply to 321671)

Илья: Там надо выводить с бонусом (reply to 321672)

Mirka: ``` пишешь это, со следующей строки код, последняя строка тоже 3 этих символа (reply to 321671)

fruitful-l: Три символа таких в начале и три в конце было бы супер поставить (reply to 321676)

Илья: Так я сделал, а оно вот так отправляется (reply to 321678)

fruitful-l: Странно  Так-то работает

Илья: Ну кто-то видит в коде что-то не то есть?

Илья: Ну скопируй мой скрипт и покажи как, я не умею. (reply to 321683)

Илья: Я добавил как говорили

fruitful-l: Второй вариант, выделить весь текст, Format, и последнее будет Code ifudjsnzb (reply to 321687)

Андрей: Весь чат в мусор превратили из за одного троля

Илья: О. Сделал

Илья: Норм?

Илья: Так кто что скажет?

Илья: М?

Илья: Заплачу сто баксов кто поможет убрать 429 ошибку

&rey: Здесь апи ключ тоже должен быть. (reply to 321676)

Илья: Типа из-за этого? Что надо написать вместо этой строки? (reply to 321698)

Combot: Combot выдал(а) предупреждение Илья (1/3) Reason: 1 reports (reply to 321694)

Илья: Норм

Grigorii: Всем привет, мне нужно зарезервировать: баланс контракта до транзакции - amount  Я пробую сделать 2 флага (+4;+8) Но я понимаю что два флага не могут друг с другом работать, так как противоречат друг другу, как я могу добиться этого результата? reserve: баланс контракта до транзакции - amount

Tim: Очень странный кейс, точно нужно? (reply to 321707)

Tim: Обычный резерв с негативным amount не помогает?

Grigorii: А есть какая то функция для получения баланса контракта до транзакции?

Tim: balance - in_amount (reply to 321710)

Grigorii: Ааа, я понял, я волновался что не будет учтен газ при отправке мессаджа со 128 sendMode, но щас вспомнил что за это отвечает сам SEND_MODE, а не raw_reserve  Спасибо

&rey: В объект добавить , apiKey: TON_API_KEY (reply to 321702)

fruitful-l: А что за фокусы с негативным amount? Как это работает? (reply to 321709)

Tim: Я бы предположил, что так же как и позитивный (reply to 321716)

fruitful-l: Может я не уловил мысль? Изначальное предложение было в отрицательном amount? (reply to 321709)

Ivan: А как на толк написать вот такую штуку   тебе приходит ячейка, в ней может быть одна рефка а может быть и две, и в зависимости от этого надо нужное кол-во send_raw_message отправить  с send_raw_message все понятно как  а на что while цикл заменить?

Petr: А зачем его заменять? Должно работать ровно также и на Толке

Ivan: и правда  а как в Tolk написать логику, что если сообщение прислали без баунс флага то вернуть все тоны?

&rey: Прочитать bounce флаг из полной ячейки сообщения. Если да, то send raw message в режиме 128 и return;  Ну и с жетонами такое не прокатит. В том числе, если жетон-кошелёк вам тоны форварднул без баунса, то вы их потеряете навсегда. (reply to 321724)

Ivan: Так а где эту полную ячейку взять, там уже все распаршено (reply to 321725)

&rey: Перед аргументом in: InMessage добавить. Будет inFull: Cell, in: InMessage. (reply to 321726)

Nick: А для чего такое нужно? (reply to 321724)

Ivan: Защищаться от школьников-ботоводов, которые на сейл контракты шлют сообщения без баунс флага и ноют что тоны застряли (reply to 321728)

Nick: try / catch? (reply to 321729)

Tim: А давайте лучше эти деньги будем собирать на отдельном адресе и сжигать на масленицу (reply to 321729)

Ivan: А в экшн фазе как? (reply to 321730)

Nick: А что за проблемы на экшен фазе? (reply to 321732)

Roman: Всем привет, может кто сталкивался с таким? Во время эмуляции транзакции через TONApi отлетает с ошибкой:iteration: 1, exitCode: 0, Text: Emulate transaction failed: [Error : -669 : cannot run message on account cannot create storage phase of a new transaction for smart contract 852443F8599FE6A5DA34FE43049AC4E0BEB3071BB2BFB56635EA9421287C…

Ivan: есть же флаг который экшн фазу может баунсить  Если его не использовать то try/catch  подойдет  Но как будто хочется ранний return делать Флаги сообщений мы же в любом случае вычитываем (reply to 321733)

Nick: Хм… как будто если есть ошибки в action фазе - значит с бизнес-логикой в compute фазе что-то не так 🤔 (reply to 321735)

Tim: Слушайте

Tim: А почему нельзя оверрайднуть баунс флаг?

&rey: Ну, тогда жетон-кошельки сломаются, например? Если ты получил трансфер нотификейшн и отбаунсил его, или что-нибудь подобное. (reply to 321739)

&rey: Контракт может рассчитывать, что с флагом bounced к нему придёт только отправленное им же ранее сообщение.

Ivan: думаешь зря добавили новый флаг? (reply to 321736)

Tim: Ну только такое и придет (reply to 321741)

&rey: я имел в виду, что придёт только отправленное им же ранее с флагом bounce=true (reply to 321743)

Tim: Я понимаю, но как минимум жетоны это не сломает

❤️🐾 🏰 Tower: Подскажите пожалуйста, как расшифровать сообщение которое было зашифровано с отправкой тон

&rey: Если жетон проверяет опкод на равенство его internal transfer, то будет нормально, иначе так себе. (reply to 321745)

❤️🐾 🏰 Tower: ? (reply to 321747)

&rey: Попробуйте https://docs.ton.org/ru/v3/documentation/smart-contracts/message-management/internal-messages#%D1%81%D0%BE%D0%BE%D0%B1%D1%89%D0%B5%D0%BD%D0%B8%D1%8F-%D1%81-%D0%B7%D0%B0%D1%88%D0%B8%D1%84%D1%80%D0%BE%D0%B2%D0%B0%D0%BD%D0%BD%D1%8B%D0%BC%D0%B8-%D0%BA%D0%BE%D0%BC%D0%BC%D0%B5%D0%BD%D1%82%D0%B0%D1%80%D0%B8%D1%8F%D0%BC%D0%B8, или там какая-то ошибка? (reply to 321747)

— 2025-09-16 —

Wayfs: Здравствуйте, подскажите пожалуйста как добавить в кошелёк usdt в сетях TON и ещё другие сети, чтоб каждому свой адрес депозита выдавал?

Wayfs: Есть кто шарит, мне это нужен для своей п2п площадки, не могу правильно настроить кошелёк пополнении и вывод

❤️🐾 🏰 Tower: Да читал, мне нужно чтобы если комментарий зашифрован, то я как-то смог его расшифровать. Не смог найти ничего 🤷 (reply to 321751)

❤️🐾 🏰 Tower: Плиз может кто делал или есть дока для чтения (reply to 321747)

Alexander: С удивлением заметил что sandbox теперь поддерживает рассчет покрытия и перемежение очереди сообщений. Круто как бы. Вот только я не понимаю как этим(coverage) пользоваться - включаю оно адски спамит в консоль, а потом показывает что  значит у меня все инструкции это  0 , SETCP(вызван 120+ раз) и DICTPUSHCONST  Тест при этом сущностно проходит - все посчиталось. Библиотек я тут не использую.  У кого-то есть положительный опыт? Может от версии зависит

/B4ckSl4sh\: Спамятся в консоль логи. Они мешают? Мне лично не мешают, но по идее можно отключить через blockchain.verbosity.print = false (reply to 321772)

Alexander: Да логи-то ладно, они просто самим фактом вывода тормозят прогон тестов. Просто удивился что они самовключились для покрытия и печатаются явно. Это-то совсем не беда я это больше как симптом что sandbox что-то пытается делать (там есть логи от от других инструкций) а не просто видит что ему не нравится кодовая страница. (reply to 321773)

Tim: Пока Тулзы для coverage еще не в релизном состоянии (reply to 321772)

Tim: Про авто включение и вывод в консоль я проверю

Alexander: Спасибо давайте я сейчас конкретику посмотрю по версиям чтобы зря время не тратить... 0.37.0 судя по всему. (reply to 321776)

Tim:         blockchain = await Blockchain.create();         blockchain.enableCoverage();         blockchain.verbosity.print = false Пока можно начать с вот такого.  А результаты у меня показываются нормально (reply to 321778)

Леша: А от чего зависит стоимость инструкции по газу (reply to 321780)

Tim: У большинства константа, у тех у которых нет, надо в доке читать (reply to 321781)

Tim: у sha256 от длины инпута, у dictget от глубины

Alexander: Спасибо, я сейчас попробую сделать минимальный тест только на основных контрактах, потому что все что я пока получаю  {   totalLines: 9440,   coveredLines: 2,   uncoveredLines: 9438,   coveragePercentage: 0.0211864406779661,   totalGas: 10481,   totalHits: 63,   instructionStats: [     { name: '0', totalGas: 9221, totalHits: 21, avgGas: 439.1 },     { name: 'DICTPUSHCONST', totalGas: 714, totalHits: 21, avgGas: 34 },     { name: 'SETCP', totalGas: 546, totalHits: 21, avgGas: 26 }   ] } (reply to 321780)

Alexander: Примерно вот так ...  но лучше не стало, хотя логов лишних - да теперь нет. сендбокс 0.37.1 блюпринт 0.40  import { Blockchain, printTransactionFees, SandboxContract, TreasuryContract} from "@ton/sandbox" import { Address, Cell, toNano } from "@ton/ton" describe("Coverage", () => {     let blockchain : Blockchain     let T : SandboxContract<TreasuryContract>       beforeEach(async () => {         blockchain = await Blockchain.create();         blockchain.enableCoverage()         blockchain.verbosity.print = false         T = await blockchain.treasury("test")     })     it("Test actual Coverage collector", async() => {                const res = await T.send({ value: toNano(1), to: Address.parse("EQAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAM9c"), body: Cell.EMPTY})         printTransactionFees(res.transactions)     })     afterAll(() => {         const coverage = blockchain.coverage(T);         console.log(coverage?.summary())     }) })

Tim: А что пишет?

Alexander: А все так же...   если включить логи TVM - то там все исполняется как надо и команд много.     {       totalLines: 37,       coveredLines: 2,       uncoveredLines: 35,       coveragePercentage: 5.405405405405405,       totalGas: 858,       totalHits: 9,       instructionStats: [         { name: '0', totalGas: 678, totalHits: 3, avgGas: 226 },         { name: 'DICTPUSHCONST', totalGas: 102, totalHits: 3, avgGas: 34 },         { name: 'SETCP', totalGas: 78, totalHits: 3, avgGas: 26 }       ]     } (reply to 321788)

Alexander: Сейчас еще сделаю минимальный проект чтобы убрать лишние зависимости.

&rey: Звучит, будто со всех инструкций газ сложился и учёлся на "0". Инструкции "0", кстати говоря, не бывает в природе. (reply to 321784)

Alexander: Вот такие вот зависимости, тот же результат. Что еще может влиять.. ну вот у меня Linux, если там реально что-то из текста парсится - то локаль  LANG=en_US.UTF-8 LC_NUMERIC=ru_RU.UTF-8 LC_TIME=ru_RU.UTF-8 LC_COLLATE="en_US.UTF-8" {   "name": "sandbox-test",   "version": "1.0.0",   "license": "Internal",    "devDependencies": {         "@swc/core": "^1.5.25",                       "@ton/sandbox": "^0.37.1",         "@ton/ton": "^15.3.1",         "ts-node": "^10.9.2",         "typescript": "^5.4.5"   },   "dependencies": {       "jest": "^29.7.0",       "ts-jest": "^29.1.5"   },   "engines": {       "node": ">=18.0.0"   } }

KotLemon: Занимаюсь безопасностью веб-приложений. Если у кого-то есть сомнения по поводу уязвимостей на своем проекте — write me. Напишу отчет по найденным проблемам.

Tim: Мне передали, что дело может быть в npm Попробуйте с pnpm (reply to 321794)

❤️🐾 🏰 Tower: . (reply to 321763)

🐼 Sild: да никто не поможет, этот вопрос не имеет никакого смысла (reply to 321801)

🐼 Sild: там может быть вообще какое угодно бинарное добро отправлено

F: Да там все просто, примеры даже где то были в каких то библиотеках Уточните хоть на каком языке вам нужно (reply to 321747)

❤️🐾 🏰 Tower: В каких? Не подскажите? Js, python (reply to 321805)

F: Реализация на го: https://github.com/xssnick/tonutils-go/blob/master/ton/wallet/wallet.go#L669  https://github.com/xssnick/tonutils-go/blob/master/ton/wallet/wallet.go#L605  Использование: https://github.com/xssnick/tonutils-go/blob/master/ton/wallet/wallet_test.go#L650  На других языках так же нужно покопаться и поспрашивать. Если найду то дополню (reply to 321806)

❤️🐾 🏰 Tower: Спасибо (reply to 321807)

⛰️ Roman ⛰️: В чем преимущество Хайлоад валлет? По коду экстернала он просто принимает сообщение с подписью и в конце отправляет добавленное сообщение. Это если не учитывать что я не понял зачем нужны queries и old queries в с4 контракта...

maksim: можно много сообщений за один экстернал отправлять и трекать это (reply to 321818)

maksim: нужно для бирж и платежных аггрегаторов всяких

⛰️ Roman ⛰️: Да, это понятно, но по коду он достаёт только 1 сообщение с 1 получателем

Tim: а если нет... (reply to 321822)

⛰️ Roman ⛰️: Может у кого то есть пример для экстернал сообщения для хайлоад валлета?

maksim: в ton/ton или в tonweb есть враппер где видно логику составления экстернала из множества интерналов, которые хочется отправить (reply to 321824)

⛰️ Roman ⛰️: Спасибо, посмотрю (reply to 321825)

maksim: вот где-то тут вероятно https://github.com/toncenter/tonweb/blob/master/src/contract/highloadWallet/HighloadWalletContractV3.js (reply to 321826)

Alexander: спасибо, пока пробовал с yarn - там вообще как-то все поломалось, но это вероятно проблема моих рук (reply to 321798)

adb: смотрел тут фикс сейл контракт ггшный, получается овнер может по этому опкоду обнулить фи. я чет не понимаю или зачем такая задумка

&rey: А это из-за проблем replay protection при обработке провала экшн фазы. (reply to 321822)

&rey: bicycle посмотрите. (reply to 321755)

Tim: Задумка в том что можно поменять цену без снятия с продажи (reply to 321838)

adb: так это выходит, я могу выставить на продажу и опкодом снять роялти и маркетплейс фи потом просто дернув метод не меняя цену (reply to 321841)
...
Даниил: Да, это правда. Автор контрактов гетгемса говорил, что так и задумано (reply to 321842)

Сигма: Как скачати

Vladimir: Скачать TON? (reply to 321855)

ISMA&COIN: Всем привет

ISMA&COIN: Кто не спит и свободен сейчас ?

Combot: Combot выдал(а) предупреждение kopoba (1/3) Reason: 1 reports (reply to 321862)

Combot: kopoba has been temporarily muted for 86400 seconds

— 2025-09-17 —

Combot: Привет, Пиши сюда @Scorpion_ZODIAC! Прочти правила и нажми кнопку ниже.

Alexander: Всем привет! Подскажите пожалуйста, если ли какой-то фасет в тестовой сети тона на usdt?

Mirka: Сминти себе жетон с 6 знаками после запятой (reply to 322106)

Alexander: Мне надо чтобы адрес с usdt совпадал Для тестов

Denis: Так не получится (reply to 322108)

Alexander: Печально( Спасибо (reply to 322109)

Василий: Такой кейс вы можете воспроизвести только в  эмуляторе. Например (reply to 322108)

Alexander: Понял, спасибо! (reply to 322113)

ZY: Всем привет. Буду рад, если кто-то сможет помочь!  В Тонкипере есть возможностость «включить» принятие/холд/отправку токенов usdt в сети trc20  Так вот, я этим постоянно пользовался. Но теперь, вкладка резко пропала  (На скриншоте , который я приложил, ранее справа сверху можно было нажать на trc20)  У меня там находятся деньги, 2.000$  Версия приложения последняя, в настройках нет функции включить trc20  Что в таком случае делать и к кому обращаться?

Max: В поддержку Tonkeeper (reply to 322116)

ZY: Функция пропала

Vladislav: Теперь нельзя батарею юзать? (reply to 322120)

Narek: Для трона? (reply to 322125)

Denis: Региональные ограничения. Ничем не можем помочь

Martin: Лучше скачайте MyTonWallet и перенесите туда кошелек через приватник.   Там есть TRC-20 и никто не будет удалять ваши адреса с деньгами. (reply to 322116)

ZY: Может в моем вопросе поможете? (reply to 322129)

Denis: Так я вам и ответил

ZY: Это все понятно. Но мне нужно теперь этот адрес достать (reply to 322130)

ZY: Региональные ограничения по юсдт? (reply to 322129)

Denis: Сид фраза общая и доступна в настройках (reply to 322133)

Denis: Есть один остров свободы (reply to 322134)

ZY: Можешь подсказать, как там трс20 включить? (reply to 322130)

🐼 Sild: Впн не спасет? (reply to 322129)

ZY: Не спас (reply to 322139)

ZY: Ну в МТВ трон с этого кошелька не подтягивается

Max: Регион стора/регион в настройках устройства (reply to 322140)

ZY: Поменял на сша не спасло (reply to 322143)

Max: нельзя на штаты

ZY: Щас другре попробую

Denis: если айфон с UK/US стором то не поможет

ZY: Это можно как то изменить? (reply to 322149)

Narek: Что за новые приколы? Можно где-то прочесть из за чего так? (reply to 322129)

Narek: С какого времени DEX контролируется правительством.

Combot: Combot выдал(а) предупреждение Mikhail Ronin (1/3) Reason: 1 reports (reply to 322161)

ZY: чисто тонкипер побрил меня на деньги

🐼 Sild: а причем тут декс и тонкипер?) Тонкипер это приложение, дистрибуция приложений на платформах, внезапно, контролируется правительством (reply to 322159)

Denis: wallet.tonkeeper.com (reply to 322157)

Max: К сожалению, это сделали регуляторы той юрисдикции, в которой вы находитесь (reply to 322164)

Denis: Хотите ржаку? Народ выдумывает опыт работы в тонапи и потом ходит по собесам в другие тоновские компании. Ну как минимум один умник нашёлся.

Vladislav: "мозгов у меня нет, но есть идея" (reply to 322169)

Narek: Так а что контролируется именно? Что нельзя со сетью трон работать или что? (reply to 322165)

Max: Само использование блокчейна не пресекается (reply to 322171)

/B4ckSl4sh\: Видимо человек только в тон пришёл, ещё не знает что тон это деревня, в которой 100 человек и все друг друга знают (reply to 322169)

Tim: В тонкипер пришел с опытом в тонапи? (reply to 322169)

maksim: в ton console (reply to 322175)

Es Ist Das: Перефразирую удалённое сообщение: вот вроде есть layerzero, но для написания кроссчейна ты либо пользуешься их func++ с кучей ограничений, либо пытаешься хакнуть формат сообщений и гоняешь по дебагу. Нативной поддержки сообщений и контрактов tonviewer нет (8 месяцев прошло с анонса), и видимо не планируется. TAC с закрытыми сорцами, что доверия сторонним билдерам не добавляет. Вопрос: это выстрел в ногу или продуманная стратегия поведения с нашей стороны?

Denis: как только мне дадут схемы так сразу и появится (reply to 322179)

Denis: но читать funC++ чтобы разобраться в этом я не готов

Tim: А есть ли там схема на тлб, или с++ имеет свою схему и для нас такая ячейка динамическая (reply to 322180)

maksim: тлб там нет, func++ основан на тюплах в основном, там отдельный фреймворк самописный для внутренней сериализации (reply to 322185)

Tim: Ну я как бы не про это (reply to 322186)

Denis: ну сообщения слава богу не тюплами ходят (reply to 322186)

QQ: Всем привет! Я столкнулся с одной очень бесящей проблемой в библиотеке @ton/ton для ts при написании парсера. Разрабатываю я в testnet и соответственно api я использую testnet.toncenter.com. При использовании любых методов, где предается адрес в формате Address библиотека не учитывая контекст для обращения к api, и преобразовывает адрес в строку { bounceable: true, testOnly: false}, но api такой формат не устраивает и она выбрасывает ошибку. Я выработал такой костыль:  await this.client.getTransactions(Address.parse(address).toString({ bounceable: true, testOnly: true }) as any, {       limit: 1,       lt: lt,       hash: hashAsBase64,       archival: true }); Но это не выглядит правильным и иногда вызывает ошибки, есть способ указать клиенту  TonClient преобразовывать адреса как мне нужно?

Denis: надо бы просто пофиксить тонцентр чтобы они все форматы принимали

Denis: https://tonviewer.com/tools/wallet (ссылка есть на главной) зацените кстати сейчас нашёл штуку в тонвьювере.

Dmitry: подозреваю это была фича. лучше либу пофиксить (reply to 322195)

Denis: плохая фича

Dmitriy: Еще б оно не ругалось когда домен вводишь  Для адресов думаю не большая проблема т.к их копировать будут (reply to 322196)

Аист: Ищу человека, который работает с TGS. Необходимо сделать пиксельный стикерпак.  Может кто знает таких?

I: Добрый день!  Мне нужно подключить кошелек пользователя по протоколу TonConnect 2.0 на бэкенде, без использования официального JS-компонента и я уже кажется немного запутался в документации.   Может кто-то проконсультировать?  Я нашел либу на го https://github.com/cameo-engineering/tonconnect для генерации deeplink. Но кажется мне еще нужно держать открытым наружу какой то эндпоинт, куда придет колбэк от кошелька?

I: Так же в официальной доке нашел sdk на python https://tonutils.ness.su/ru/cookbook/tonconnect-integration/, но почитав код, вижу что там вообще про другое. (reply to 322204)

Tim: https://github.com/ton-blockchain/ton-connect Тон коннект работает через бриджи, никакие эндпоинты ни с чьей стороны ничего не слушают (reply to 322204)

I: Спасибо, изучу (reply to 322207)

Ilya: добрый день, а есть ли такая группа только для веб апп телеграм?

Mirka: https://t.me/devs? (reply to 322211)

Es Ist Das: Правильно понимаю, что схемы нужны вот в таком формате? https://github.com/tonkeeper/tongo/blob/master/abi/schemas/bemo.xml (reply to 322180)

Denis: ну это идеально.  но в принципе хоть в какому-то (reply to 322216)

「Ж」 VXK PROD: Привет, друзья! Как лучше всего получить цену токена в сети TON? Ищу надёжный способ (API, SDK или on-chain метод). Буду благодарен за любые рекомендации.

🐼 Sild: https://api.ston.fi/swagger-ui/#/Dex/get_asset_list тут вот целых 4 поля говорят  dex_price_usd dex_usd_price third_party_price_usd third_party_usd_price  правда только токены которые там листились (reply to 322224)

Andrey: https://dton.io/api/v2/assets/token_info/usdt-minter.ton (reply to 322224)

— 2025-09-18 —

Alexander: Подскажите пожалуйста, какие версии кошельков сейчас реально в основном используются? V3 V4 или W5? Надо код писать, а большого опыта нет в тоне 😁

X1ag: v4r2, w5, highloadv3 (reply to 322240)

Alexander: Благодарю (reply to 322241)

Oleg: Ну и V3 конечно же, ибо V4 хуже во всем, кроме поддержки плагинов, а они, насколько мне известно, на данный момент не используются совсем (reply to 322242)

cb: Ощущение, что эта штука в тон коннект перестала выдерживать. Там где с гитхаба тянется инфа.

&rey: Надо бы выложить его и на ton.org) (reply to 322257)

Combot: Combot выдал(а) предупреждение Головин (1/3) Reason: 1 reports (reply to 322259)

cb: Да, туда бы мощностя на этот файл (reply to 322258)

Tim: Давно надо (reply to 322258)

Tim: Это не имеет смысла (reply to 322275)

cb: Ещё такая ошибка есть

Tim: Так а что ломается от этого? (reply to 322279)

cb: Грубо говоря, если загрузка аппки завязана на подгрузке этого пакета, то не загрузится аппка, а так вообще, это модуль аналитики ton.builders. Не знаю насколько это распространено, но у меня также не работает TON ID, необходимый для подключения к ton.builders. Уже около полугода. Около недели назад работал в течение нескольких дней. (reply to 322280)

Tim: Так а почему загрузка аппки должна быть завязана на загрузку этого пакета? Он же должен быть опциональным

cb: Я не говорю, что обязательно, это если пакет таким образом в фронт вшит, что влияет на загрузку

&rey: Хотя почему даппки не хостят список кошельков сами? (reply to 322258)

cb: Может быть кто-то так и делает, но в общем случае, этот URL в TON Connect SDK содержится, который устанавливается менеджером пакетов. Грубо говоря этот линк не в коде, который пишет разработчик, а в пакете, который является зависимостью. (reply to 322284)

&rey: Минусы: не обновить центрально. Плюсы: не обновить центрально, не отследить всех пользователей всех приложений. (reply to 322284)

zeroxxx: добрый день, хотел бы узнать как на клиенте создать транзакцию не в TON, а в другой валюте, в документации есть пример: const transactionWithExtraCurrency = {     validUntil: Math.floor(Date.now() / 1000) + 60,     messages: [         {             address: "EQBBJBB3HagsujBqVfqeDUPJ0kXjgTPLWPFFffuNXNiJL0aA",             // Specify the extra currency             extraCurrency: {                 100: "10000000"             }         }     ] }; но не понятно что подразумевается в массиве extraCurrency ключ 100

Denis: экстракаренси есть только в тестнете и если у вас есть такого уровня вопросы то у вас их нет

zeroxxx: а как в мейннете сделать отправку монеты USDT через tonconnect?

5b 44 43 54 5d: https://docs.ton.org/v3/guidelines/ton-connect/cookbook/jetton-transfer 🤨 (reply to 322293)

DPL: Как можно найти данные мастер блока имея все данные транзакции?

🐼 Sild: ххе, а в транзакции нет информации о каких-нибудь мастер-блоках

&rey: См. https://toncenter.com/api/v3/index.html#/blockchain/api_v3_get_transactions. (reply to 322302)

&rey: Там будет mc_block_seqno, а другие данные блока можно загрузить отдельно.

DPL: Спасибо (reply to 322304)

Andrew: Ребят, вопрос такой, что делать с данной ошибкой? {'error': 'LITE_SERVER_UNKNOWN: cannot apply external message to current state : External message was not accepted\nCannot run message on account: inbound external message rejected by transaction 39F796F267B5E798719DF9F8B989ED3AB09FE9D11465ABAE27ACF8C267419A73:\nexitcode=0, steps=0, gas_used=0'

&rey: Кошелёк задеплоен уже? (reply to 322310)

&rey: (Конечно, либа сама должна это делать при seqno=0, но вдруг пропустила.)

Andrew: Да (reply to 322311)

X1ag: А есть те кто пользовался Streaming API от Tonapi? почему sse листенер может раз в несколько суток просто выключаться?

Tim: ну выключился и выключился, можно же с того же момента подключиться?  А вообще проверяй энкодинги, чанки, кипалайвы (reply to 322314)

Denis: Эм. Тоесть типа через сутки коннект рвет и надо переподключаться? (reply to 322314)

X1ag: реконнект как в примерах tonapi-go есть, и он работает, в какой то момент транзы перестают приходить (reply to 322316)

Denis: Интересно. Мы посмотрим

Arsen: Кто может помочь распарсить полученный с фронтенда boc из external message?  Язык python Задача: Приходит boc с фронтенда на бэк, мне нужно распарсить boc, найти нужную транзакцию и определить её статус(успешна, нет)

&rey: В этом боке нет даже доказательства, что цепочка транзакций началась, всё равно в чейн надо смотреть. (reply to 322327)

Arsen: Да я просто вообще не дружу с ton connect и со всем, что связано с блокчейном, криптовалютой и т.д., но прилетела задача подключить в mini app web3 и пополнение баланса через него Фронтендер отправляет мне boc, и я пытаюсь понять, как достать эту транзакцию и посмотреть её статус. В доках нашёл пример только на js https://docs.ton.org/ru/v3/guidelines/ton-connect/guidelines/transaction-by-external-message#finding-a-transaction-by-incoming-message

NFT: Ты можешь распарсить BOC с помощью библиотеки pytonlib, важно понимать, что BOC, который приходит с фронтенда, обычно представляет собой неподписанное или подписанное сообщение (транзакцию), предназначенное для отправки в сеть, а не результат уже произошедшей транзакции. Поэтому статус (успешна или нет) нужно проверять уже после отправки. Сначала нужно преобразовать строку BOC, которую ты получил с фронтенда (скорее всего, это base64-строка), в объект Cell. Это основной строительный блок в TON. После того как ты распарсил BOC, чтобы узнать статус транзакции, тебе нужно: Отправить этот BOC в сеть TON через клиент. Получить хэш транзакции. Запросить информацию о транзакции по её хэшу у ноды TON. Для этого тебе нужен TON Client. Статус транзакции не содержится в самом BOC, используй Cell.from_boc(base64.b64decode(boc_string)), чтобы получить объект Cell. Чтобы определить статус, тебе нужно отправить BOC в сеть TON (с помощью TonlibClient или аналогичного) и затем проверить статус транзакции в блокчейне по её хэшу.... (reply to 322327)
...
Grigorii: Всем привет, а есть какие то готовые опенсорсные решения парсинга транзакций в блокчейне на golang/python? И если есть, то где их найти?

X1ag: hi, а у меня одного выскакивает ошибка  "error":"all liteservers are unavailable"  в либе tonkeeper/tongo когда использую их liteapi liteapi.NewClientWithDefaultTestnet()  и  liteapi.NewClientWithDefaultMainnet() ?

Denis: УМВР (reply to 322478)

X1ag: а почему тогда может быть такая ошибка? Куда смотреть? (reply to 322479)

Denis: ну видимо все ЛС недоступны

X1ag: но у вас же доступны) (reply to 322481)

Petr: Тем кому надо, тем доступны 😄 (reply to 322482)

Denis: ну я петька и постарше буду (reply to 322482)

Оскар: в чате тон провайдера поднималась похожая тема, пришли к выводу что по каким-то причинам из россии часто коннект к публичным лайтсерверам не проходит совсем и только через раз куда-то да пройдет (reply to 322480)

X1ag: понял, спасибо! (reply to 322486)

Artur: Привет!  Кто-нибудь знает поддерживает ли minter.ton.org стандарт Jetton 2.0?

Tim: Поддерживает понятие относительное. Минтит он старые жетоны (reply to 322489)

Grigorii: Всем привет, а как получить все блоки за сегодня в блокчейне?

&rey: Проще всего начать с текущего мастерчейн-блока, а от него идти по всем ссылкам (prev mc, shards). (reply to 322521)

— 2025-09-21 —

Omen: Здравствуйте, можно ли username телеграм сделать суб username?

Николай: Всем привет! Подскажите, пожалуйста, куда сейчас можно подать заявку на верификацию джеттона? Раньше это делалось через репозитории ton-society/ton-verified и ton-blockchain/ton-tokens на GitHub, но сейчас они заблокированы, и я не могу убрать статус «НЕ ПРОВЕРЕН» в кошельке wallet.  Есть ли новый репозиторий или альтернативный способ проверить токен? Спасибо!

Denis: Оно всегда было через github.com/tonkeeper/ton-assets (reply to 322543)

Николай: спасибо (reply to 322544)

cb: Можно ли доверять компании Big Bang?

Jzuss ┌( ಠ_ಠ)┘: Кто-нибудь знает контакты поддержки toncenter?

Vladimir: @ toncenter_help_bot (reply to 322548)

ytiruces: Добрый день. Тестнет на выходной ушел? 503 Service Unavailable

Grigorii: Всем привет, а я правильно понимаю что в блоке максимум 256 транзакций?

A: Подскажите долго ли будет идти USDT TON?  Отправил 1 бакс , газа было вроде около 0.02TON.  В explorer видно что списалось с HOT нужная сумма, но на получателя спустя час так и не пришла

Mirka: Должно было прийти секунд за 20 максимум. Скинь транзакцию если хочешь, чтобы разобрались (reply to 322567)

A: Адрес получателя  https://tonscan.org/address/UQCf5HRwSExl7IjlNsSswJg4ERVyNfPX4xKqsOUak8nB6ABy#events   Адрес HOT кошелька, 2 транзакции подряд последние не пришли. https://tonscan.org/address/UQDCpN9b54LTvf54G1_5nCn0nrN-yTUa8mM6pNgG04MLBUhY (reply to 322568)

A: Для TON так и есть, а для USDT TON я чет не пойму в чем дело (reply to 322568)

X1ag: кошелек не создан. Нужно с этого кошелька отправить транзакцию куда-то (reply to 322570)

A: Бро, ты про деплой?  Если я не ошибаюсь  Ведь для HOT видно в explorer что я засылал на получателя 1 и 1.6 usdt.  Или что я мог не так сделать? (reply to 322572)

X1ag: с HOT кошелька отправить TON транзакцию какую нибудь. Даже самому себе. Но странно что оно не показывается с Uninit кошельком (reply to 322573)

❤️🐾 🏰 Tower: UQBLNkpxAmNfO43W-FZOaxg4rnsNo7T2J48FoeQlTcAtqjhv  На нем же usdt лежат, а что бы он был активен ответ выше (reply to 322573)

A: Тобешь нужно именно TON транзакцию ?  Не USDT? (reply to 322574)

A: Получается USDT которые я заслал ушли в никуда да?

❤️🐾 🏰 Tower: Да именно тон, для того чтобы был активен (reply to 322576)

X1ag: UQCf5HRwSExl7IjlNsSswJg4ERVyNfPX4xKqsOUak8nB6ABy  вот с этого кошелька нужно тон отправить куда нибудь (reply to 322576)

❤️🐾 🏰 Tower: UQBLNkpxAmNfO43W-FZOaxg4rnsNo7T2J48FoeQlTcAtqjhv  Они тут. Если есть сид, то переведи себе обратно (reply to 322578)

A: Понял, пробую, Сяб .  Но у меня вопрос, в контексте - production как правильно это реализовать?  Именно активацию HOT.  Есть какая то единственно верная практика? (reply to 322579)

X1ag: Я не знаю что такое HOT поэтому не подскажу (reply to 322582)

A: Горячий кошелек (reply to 322583)

A: Именно горячий кошелек биржи. Проект - крипто биржа в телеге

X1ag: с помощью кода создать кошелек - узнать его адрес, кинуть на него тоны и кинуть с этого кошелька любую транзакцию, чтобы активировать его (reply to 322585)

Леша: Задеплоить кошелек можно и не только автору кошелька (reply to 322586)

Леша: Как по мне будет неприятно деплоить каждому кошелек, а что если будет наплыв ботов, которые будут создавать аккаунты и все деньги уйдут на комиссии (reply to 322585)

Denis: Нет. Может быть больше. Самое большое реально было емнип около 450 (reply to 322560)

&rey: Единственно верной? Не думаю.  Если у вас там как-нибудь затесался wallet v5 в качестве горячего, то вы сможете юсдт перевести gasless-транзакцией, предоставляя тоны с ядра биржи. (reply to 322582)

A: v4 вроде если я понял о чем вы. (reply to 322592)

&rey: Рекомендую поменять на один из v3, v5, или preprocessed-v2. От v4 смысла бирже ноль. (reply to 322593)

🐼 Sild: Почему такой вывод? Вроде не должно быть (reply to 322560)

Tim: Все еще зависит от ваших требований. Многие биржы используют 1 кошелек для приема всех платежей с мемо (reply to 322582)

— 2025-09-22 —

BIN: Приветствую, есть задача сделать NFT коллекцию и минт айтемов. Я как бы нашел контракты на коллекцию и nft-item от GetGems.  И по сути в проде это выглядело бы как: вызов nft-item для создания айтема -> отправка айтема тому, кто минтит  Существует ли какой-то более оптимальный способ организации минта, например, чтобы юзер при минте сам вызывал контракт, оплачивал заранее прописанную сумму и получал айтем?

BIN: А вообще меня просто очень заинтересовала последняя реализация гетгемса с фрагментированием и минтом фрагментов, хотелось потыкать чет такое

User: https://github.com/ndrblinov/ton-nft-deployer  Немного древняя, но удобная штука для массового минта (reply to 322631)

BIN: Это я так понимаю оно не должно будет работать с wallet_v5r1?

&rey: Ну, можно несложно пропатчить, тогда работать будет. (reply to 322636)

BIN: Ну там по сути как я понимаю оно чисто в if'ах и константах

BIN: А остальное, я врядли смогу адаптировать и как будто бы не должно аффектить

BIN: https://ton-collection-edit.vercel.app/deploy-collection еще такое нашел

⛰️ Roman ⛰️: У меня есть сервис для создания нфт и коллекций, но работает пока только в тестнет. Могу в лс скинуть (reply to 322644)

BIN: А поч в мейннет не работает? (reply to 322645)

⛰️ Roman ⛰️: Руки пока не дошли (reply to 322646)

BIN: Кидай

наз: всем ку, подскажите тут вроде кто то когда то собирал репу в которой поясняется как работают меркл пруфы, ни у кого случаем ссылки не осталось?

Dmytro: держи (reply to 322651)

наз: не, там была именно репа с примерами (reply to 322652)

Alexey: не репа, но лично мне это видео, очень помогло в понимании меркл пруфов  https://www.youtube.com/watch?v=2kPFSoknlUU (reply to 322651)

⛰️ Roman ⛰️: кто нибудь, пожалуйста, может в несколько слов объяснить логику контракта кредитования?

&rey: Такого контракта на тоне, скорее всего, не бывает? Или какой вы имеете в виду? (reply to 322658)

⛰️ Roman ⛰️: В интернете прочитал что могут быть DeFi с кредитованием. Мне интересно какая логика у контракта (reply to 322659)

Anthony: Я думаю, что речь про Evaa и Affluent (reply to 322659)

Ssi: ну, там залог, все дела) (reply to 322660)

Андрей: Есть специальный контракт для валидаторов который может взять кредит у мастер контракта и позволяет сделать ставку в электор. Других схем кредитования без участия электора быть не может) (reply to 322659)

Ssi: не уверен, как в Тоне, но можно посмотреть на эфировские Compound и Aave протоколы

Anthony: https://docs.affluent.org/concept (reply to 322661)

&rey: Почему же? Для доверенного круга почему бы и нет))) (reply to 322663)

наз: подскажите почему 9 экзит код отдает коллекция   () mint(slice user, int quantity, int query_id, int index) impure inline {     cell nfts = new_dict();     repeat (quantity) {         cell content = begin_cell()             .store_slice(user)             .store_ref(begin_cell().store_int_as_string(index).store_slice(".json").end_cell())             .end_cell();         slice value = begin_cell()             .store_coins(const::deploy_value)             .store_ref(content)             .end_cell()             .begin_parse();         nfts~udict_set(64, index, value);     }      cell msg = begin_cell()         .store_uint(BOUNCEABLE, 6)         .store_slice(data::collection_address)         .store_coins((const::fees + const::deploy_value) * quantity)         .store_uint(0, 107)         .store_op(2)         .store_query_id(query_id)         .store_ref(nfts)         .end_cell();          send_raw_message(msg, SEND_MODE_PAY_FEES_SEPARATELY); }   if (op == 2) { ;; batch deploy of new nfts       int counter = 0;       cell deploy_list = in_msg_body~load_ref();       do {         var (item_index, item, f?) = deploy_list~udict::delete_get_min(64);         if (f?) {           counter += 1;           if (counter >= 250) { ;; Limit due to limits of action list size             throw(399);           }            throw_unless(403 + counter, item_index <= next_item_index);           deploy_nft_item(item_index, nft_item_code, item~load_coins(), item~load_ref());           if (item_index == next_item_index) {             next_item_index += 1;           }         }       } until ( ~ f?);       save_data(owner_address, next_item_index, content, nft_item_code, royalty_params);       return ();     }  () deploy_nft_item(int item_index, cell nft_item_code, int amount, cell nft_content) impure {   cell state_init = calculate_nft_item_state_init(item_index, nft_item_code);   slice nft_address = calculate_nft_item_address(workchain, state_init);   builder msg = begin_cell()             .store_uint(0x18, 6)             .store_slice(nft_address)             .store_coins(amount)             .store_uint(4 + 2 + 1, 1 + 4 + 4 + 64 + 32 + 1 + 1 + 1)             .store_ref(state_init)             .store_ref(nft_content);   send_raw_message(msg.end_cell(), 1); ;; pay transfer fees separately, revert on errors }

⛰️ Roman ⛰️: Так никто сразу не ответит, проверьте на каком опкоде ломается контракт. Максимум могу предположить что он ломается, неправильно доставая ваш dict при батч минте. (reply to 322667)

наз: я скинул метод которым отправляю на контракт коллекции опкод 2 и скинул реализацию опкода 2 (reply to 322668)

X1ag: А транзакция в тестнете есть? (reply to 322669)

наз: я пока ток в сэндбоксе (reply to 322670)

X1ag: в тестах? (reply to 322671)

наз: da (reply to 322672)

X1ag: ну логи tvm тогда где падает (reply to 322673)

наз: console.log     execute SETCP 0     execute DICTPUSHCONST 19 (xC_,1)     execute DICTIGETJMPZ     execute POP s3     execute CTOS     execute LDU 4     execute SWAP     execute PUSHINT 1     execute AND     execute PUSH s3     execute SBITS     execute OR     execute LESSINT 32     execute PUSHCONT x5F03     execute IFJMP     execute LDMSGADDR     execute DROP     execute CALLDICT 26     execute SETCP 0     execute DICTPUSHCONST 19 (xC_,1)     execute DICTIGETJMPZ     execute PUSH c4     execute CTOS     execute LDU 256     execute SWAP     execute SETGLOB 1     execute LDREF     execute SWAP     execute SETGLOB 2     execute LDU 64     execute SWAP     execute SETGLOB 3     execute LDU 64     handling exception code 9: cell underflow     default exception handler, terminating vm with exit code 9        at SmartContract.runCommon (node_modules/@ton/sandbox/dist/blockchain/SmartContract.js:268:21)    console.log     execute SETCP 0     execute DICTPUSHCONST 19 (xC_,1)     execute DICTIGETJMPZ     execute POP s3     execute CTOS     execute LDU 4     execute SWAP     execute PUSHINT 1     execute AND     execute PUSH s3     execute SBITS     execute OR     execute LESSINT 32     execute PUSHCONT x5F03     execute IFJMP     execute LDMSGADDR     execute DROP     execute CALLDICT 26     execute SETCP 0     execute DICTPUSHCONST 19 (xC_,1)     execute DICTIGETJMPZ     execute PUSH c4     execute CTOS     execute LDU 256     execute SWAP     execute SETGLOB 1     execute LDREF     execute SWAP     execute SETGLOB 2     execute LDU 64     execute SWAP     execute SETGLOB 3     execute LDU 64     execute SWAP     execute SETGLOB 4     execute LDMSGADDR     execute SWAP     execute SETGLOB 5     execute LDMSGADDR     execute SWAP     execute SETGLOB 6     execute LDU 64     execute SWAP     execute SETGLOB 7     execute LDREF     execute SWAP     execute CTOS     execute LDU 32     execute SWAP     execute SETGLOB 8     execute LDU 32     execute SWAP     execute SETGLOB 9     execute ENDS     handling exception code 9: extra data remaining in deserialized cell     default exception handler, terminating vm with exit code 9 (reply to 322674)

X1ag: теперь нужно найти где смарт контракт делает load_uint(64)

наз: оно до парсинга дикта дошло? (reply to 322676)

X1ag: да (reply to 322677)

Petr: А вы не пробовали запустить дебаг раз вы на FunC пишите?

наз: debugLogs? (reply to 322679)

Petr: Не не не, в blueprint есть дебаг FunC, вроде в конфиге компиляции надо проставить флаг

наз: чет не вижу у них такого (reply to 322681)

Petr: Ставите blockchain.debug = true и в CompilerConfig debugInfo: true, затем запускаете тест и в VS Code команду TON: Debug Contract

наз: а это какой экстенш нужен? (reply to 322683)

Petr: https://marketplace.visualstudio.com/items?itemName=ton-core.vscode-ton (reply to 322684)

Оскар: судя по тому что падаем среди SETGLOB, могу предположить что дата контракта по глобальным переменным раскидывается, и во время парсинга даты контракта что-то лежит в ней не так (reply to 322675)

Petr: Или если в курсоре то: https://open-vsx.org/extension/ton-core/vscode-ton (reply to 322685)

наз: https://github.com/ton-blockchain/token-contract/blob/main/nft/nft-collection-editable.fc  вот эту коллекцию ес че юзаю, ну там ток при парсинге квери айди load_uint(64) я его вроде кладу в mint()

Оскар: только я не особо вижу где этот контракт глобальные юзает, вероятно проблема в другом контракте? (reply to 322688)

okolo: Доброго, народ! У меня есть init в контракте, который принимает переменные. Когда я писал контракт, я предполагал, что эти переменные будут задаваться при деплое: contract WalletContract {     owner: Address;     paused: Bool;     balances: map<Address, Int as coins>;     jettonMaster: Address;     withdrawFeeBps: Int;      init(ownerAddr: Address, jettonMasterAddr: Address, withdrawFeeBps: Int) {       self.owner = ownerAddr;       self.paused = false;       self.balances = map<Address, Int as coins> {};       self.jettonMaster = jettonMasterAddr;       self.withdrawFeeBps = withdrawFeeBps;     }   } Я думал, что эти переменные можно будет передать в скрипте деплоя: export async function deployWallet(provider: NetworkProvider, owner: string, jettonMaster: string) {     const wallet = provider.open(       await Wallet.fromInit({         ownerAddr: new Address(owner),         jettonMasterAddr: new Address(jettonMaster),         withdrawFeeBps: 100, // 1%       })     );   } Однако fromInit ссылается на .ts файл, сгенерированный билдом, и там нет возможности передать нужные мне переменные: static async fromInit() {     const __gen_init = await Wallet_init();     const address = contractAddress(0, __gen_init);     return new Wallet(address, __gen_init);   } Как правильно передавать эти переменные ownerAddr и jettonMasterAddr?  я не нашёл чёткой информации — нужно ли их хардкодить прямо в контракте? Подскажите пожалуйста

наз: неа, судя по тестам в сендбоксе на контракт коллекции улетает сообщение и он возвращает 9 exit code (reply to 322689)

наз: хотя ща я лучше попробую в тестнете (reply to 322691)

Оскар: тогда без понятия, я в контракте коллекции не вижу где глобальные используются, а в дампе они есть так же в дампе нет никаких следов что контракт дожил до сравнения с опкодом 2 (reply to 322691)

наз: в точку, ток не при парсинге, а при сторе (reply to 322686)

наз: всем спасибо

Viacheslav: Привет. Мне нужно возвращать юзерам intы в get методе, но я не знаю зарание сколько чисел будет - поэтому просто get функцию написать не могу..  Будет так: овнер отправляет транзу в контракт с числом(например 3), контракт в цикле вызывает генератор рандома и записывает в сторадже выпавшие числа. И потом юезрам их нужно покатаь в get методе.. как бы такое сделать?

Оскар: самый простой вариант - гетметод отдаёт только ячейку, эта ячейка работает как связный список - в ней число и реф на следующий элемент списка, если рефа нет - значит список закончился (reply to 322702)

&rey: tuple возвращайте, значит. (reply to 322702)

Viacheslav: это нужно парсить на клиенте.. а мне бы так, для простых юзеров,которые только тонвьюэр умеют пользоваться) (reply to 322703)

Оскар: гетметоды не умеют динамически изменять кол-во возвращаемых значений да и всё равно сомневаюсь что простым юзерам будет понятно ввести название гетметода в специальной вкладке где им просто вывалят кучу интов, тут уже смотря насколько простым (reply to 322705)

&rey: Ну, тогда действительно tuple сделайте. Он вроде адекватно отображается. (reply to 322705)

Viacheslav: а сколько он может содержать максимум чисел?

Оскар: а, да, он с гетметодами нормально работает? (reply to 322707)

&rey: 255.  Если надо ещё больше чисел вывести, тогда понадобится магия ассемблера (поставить на гет-методе неверный тип возврата, а все числа выкинуть на стек; но тех чисел больше 255 сложно в стопку ячеек сложить). (reply to 322708)

Teers k 🐾: У меня к вам вопрос. Предоставляете ли услугу написания смарт контракта по заказу? Если возможно. Или где можно искать специалистов (reply to 322711)

Viacheslav: а можно на лету создавать tuple?  например recv_internal функция приняла число 5, и в for 5 раз запрашивает rand, записывает это в tuple и потом в сторадж сохраняет контракта? (reply to 322707)

&rey: В сторейдж нельзя, всё остальное можно. (reply to 322713)

— 2025-09-23 —

Alexander: Добрый день. Не подскажете какой-нибудь сервис или способ для отслеживания транзакций? Нужно знать если на кошелек или с кошелька произошла транзакция, предполагая, что у нас есть список адресов, которые нас интересуют. Я нашел публичный кафка топик для Тона, с которого вроде как можно получать транзакции почти в реальном времени, но вижу тут только одно поле для аккаунта в AVRO схеме, кто-то работал, там видны оба адреса откуда и куда?

Vladimir: tonapi / toncenter. (reply to 322750)

Danya: Дорого обойдется real life данные получать (reply to 322750)

Шикльгрубер: Привет, подскажите через что (какую библиотеку) лучше и проще делать нфт коллекции? Через гетгемс? Может есть какие то гайды, а то на ютубе мало видео по тону

𝗞𝗶𝗻𝗴 𝗖𝗿𝗶𝗺𝘀𝗼𝗻 44: Добрый день,  Можете подсказать где или как я могу получить список топ 10,000 кошельков тон по балансу?

Vladimir: https://tonscan.org/whales (reply to 322761)

DPL: Есть ли паблик liteserver archival nodes?
...
💵: Добрый вечер помогите пожалуйста с тоном , я не понимаю что делать…

Mirka: Покупать... А если серьезно, то для помощи нужны подробности (reply to 322789)

𝗞𝗶𝗻𝗴 𝗖𝗿𝗶𝗺𝘀𝗼𝗻 44: Блин, только щяс увидел, там всего 1000, можно ли где то взять 10,000 или больше? (reply to 322762)

.: возможно ли через Toncenter Streaming API подписаться на коллекцию полностью нужных мне NFT и мониторить транзакции по NFT? самих NFT может быть боле 50к. (почему сейчас пока такая логика) речь про фрагмент, а у них нет отдельного контракта, там данные внутри самого NFT обновляются.  сейчас как вариант - подписаться на кошельки где эти NFT - но их сотня

Viacheslav: если просто отправлять тон на адрес смарт контракта через кошелек, то in_msg_body будет пустое и сработает этот if? или это для других случаев?    if (in_msg_body.slice_empty?()) { ;; ignore empty messages     return ();   }

Оскар: сработает (reply to 322809)

Дядя Вова: Если на Tact и не сильно навороченный надо, то пиши в личку ) (reply to 322712)

— 2025-09-24 —

Александр: Нужен бот который будет контролировать доступ к платной телеграм-группе с темами. В нем должны быть кнопки: купить подписку на месяц, на год, пожизненный доступ, партнёрская программа, Инструкции. Перед окончанием срока подписки, бот должен оповестить подписчика о необходимости продлить, а если не оплачено продление доступа - удалить пользователя из Telegram-группы. Принимать оплату буду через платежную систему в рублях и в криптовалюте TON. Кто может сделать данный бот?

Daniel: Делал подобного бота, можете написать в ЛС (reply to 322818)

Виктор 👺: А tribute не сойдет? (reply to 322818)

Александр: Это что tribute? (reply to 322822)

Виктор 👺: Бот ровно с таким же функционалом (reply to 322823)

Artem: Привет! Подскажите какое время жизни подписанной транзакции(сообщения) в сети? UPD нашел, что оно указывается в сообщении

наз: всем ку, а если ключи дикта адреса то как правильно их доставать?   cell dict = ps~load_ref();         (slice entry, int found?) = dict.udict_get?(267, sender~load_uint(267));         throw_unless(45, found?);  вот это отдает 5 экзит код

fruitful-l: Uint 267? (reply to 322856)

наз: Dictionary.empty(Dictionary.Keys.Address(), Dictionary.Values.BigUint(256)); (reply to 322859)

Combot: Combot выдал(а) предупреждение Есений (1/3) Reason: 1 reports (reply to 322858)

fruitful-l: Так 5 экзит код из-за этого наверно? (reply to 322856)

наз: ну я и спрашиваю как правильно доставать (reply to 322863)

Alex: Подскажите пожалуйста, через какой сервис лучше взаимодействовать с блокчейном тона для отправки транзакций? Пробую через toncenter, почему-то не проходит (пробую без ключа) Через tonapi норм, но не понимаю, насколько он распространён

Anthony: Tonapi – tier-1 API провайдер в ТОН (reply to 322865)

Anthony: А какая ошибка? (reply to 322865)

Alex: Благодарю! (reply to 322866)

Anthony: https://tribute.tg/ (reply to 322823)

Alex: Error: toncenter HTTP 500: {"ok":false,"error":"LITE_SERVER_UNKNOWN: cannot apply external message to current state : failed to parse external message: external message must begin with ext_in_msg_info$10","code":500} (reply to 322867)

F: Это ошибка говорит что сообщение не корректно составлено Звучит невероятно что через тонапи транзакция проходит, а через тонцентр выдает такую ошибку Предположу что либо тонапи просто проглатывает и игнорит ваше сообщение, либо вы как-то это дополнительно форматируете для тонценра не так (reply to 322870)

Alex: Понял, спасибо

Оскар: у словаря есть _get для ключа-слайса, но по каким-то причинам в stdlib решили только запихать функцию для set по ключу-слайсу, нужно самому добавить эту функцию  (slice, int) dict_get?(cell dict, int key_len, slice key) asm(key dict key_len) "DICTGET" "NULLSWAPIFNOT"; (reply to 322856)

наз: пасиба (reply to 322874)

Дядя Вова: Вопрос  нужны примеры смарт-контрактов на языке  Tolk:  1. нфт по стандарту 2.0 2. токенов по стандарту Jetton 2.0  Кто знает где смотреть, подскажите )

Denis: нфт по стандарту 2.0 ничем не отличаются от 1.0 с точки зрения смартконтрактов. стандарт жетонов 2.0 это видимо любой контракт жетонов с динамическим расчётом газа

&rey: dict.dict_get?(267, sender)  А если этот метод не найдётся, то проще уже переехать на тот язык, где есть. На Толк. (reply to 322856)

наз: да давно пора, пока руки не доходят (reply to 322880)

🐼 Sild: там же еще воллет в шард овнера попадать должен, газом не обойдется (reply to 322879)

Вика: Привет всем, а можно ли в тестах sandbox эмулировать транзакции от реальных адресов?  То есть в evm и foundry есть vm.prank и туда можно любой адрес вставить, а можно ли что-то подобное провернуть в ton?

Дядя Вова: извиняюсь, но вы самое вкусное проспали ))) тысячу извинений ))) такое мог сказать только человек, который слыхом не слышал ни о нфт 2.0, ни о jetton 2.0^ а тем более о новом языке Tolk 😎  еще раз - тысячу извинений 🤝🤝🤝 (reply to 322879)

Mirka: Выписка из документации нфт2.0: The NFT contracts themselves remain unchanged (reply to 322884)

Дядя Вова: нфт 2.0 на Tact я умею ) меня интересует нфт 2.0 на Tolk ) (reply to 322885)

maksim: +шардинг по юзер кошелькам еще вроде (reply to 322879)

Denis: прости я дальше в спячку :) (reply to 322887)

Дядя Вова: ню-ню ))) (reply to 322887)

Дядя Вова: судя по реакциям - здесь точно чат Разработчиков? )))  вы уже хотя бы на Tact запрограммировали на век вперед стоимость вашего жетона в экосистеме тон, а в нфт на тон впихнули всё что можно (и даже то, что нельзя ;) ??? ))) (reply to 322889)

sd: какой-то залетный троль

ㅤ: Привет друзья. Можно ли как то сделать передачу подарков через бота? Чтобы я передал, а он по команде отправил другому пользователю?  Такая функция есть, но как я понял не у всех ботов

Vladimir: Чат русскоязычного сообщества разработчиков на TON. (reply to 322894)

ㅤ: Я не спрашиваю реализации, мне нужен метод (reply to 322897)

ㅤ: Или это оффтоп?

Оскар: вероятность что помогут в этом чате выше @ devs_cis (reply to 322894)

Dans12: Всем привет! Подскажите пожалуйста какой библиотекой на Python лучше пользоваться для управления тоном и тп приколами. Я читал документацию от тона о tonutils, она подходит для первого мини проекта?

alkhanov: Очень даже (reply to 322927)

alkhanov: Дальше можешь посмотреть tonsdk и pytonapi

💵: Если у меня есть тон я могу себе звезды купить?

Виктор 👺: Да, можно. Split tg (reply to 322931)

CodWiz: фрагмент лучше

fruitful-l: У будущих версий TVM есть какие-то ориентировочные даты, которые известны заранее, или это всегда сюрприз для большинства?

Виктор 👺: Фрагмент требует паспорт вроде (reply to 322933)

CodWiz: и чо? (reply to 322939)

Виктор 👺: Ахаха, ну хочется криптой все таки анонимно пользоваться (reply to 322940)

Виктор 👺: И не сливать рандомной компаний паспорт только потомучто

Виктор 👺: Чтобы она еще и просрала его, если вдруг у них дырявая архитектуиа

CodWiz: а где ты в 21 веке увидел "анонимность"?  щас есть только приватность (reply to 322941)

Petr: Насколько я знаю прям графика нет (reply to 322938)

Petr: Но можно следить за ветками в монорепе ТОНа, при появлении новой версии она появится там, потом смержится и через время появится у всех в мейннете

fruitful-l: Спасибо! (reply to 322946)

Petr: А ну и перед мейннетом появится в тестнете

Petr: TVM 12 вроде планируется ближе к середине октября

Виктор 👺: Есть локальная анонимность. Конечно, если прям сильно захотеть, то можно много что восстановить из блокчейна, но хочется максимально усложнить этот процесс всем, кто не так сильно хочет. (reply to 322944)

Petr: Его уже слили госуслуги 😄 (reply to 322942)

Виктор 👺: Ну вот за слив через госуслуги хоть кто то получил по башке, а вот фрагмент, ну хз. Я же типо сам им отдал свой паспорт (reply to 322951)

Viacheslav: Привет.  вопрос по tolk. если у меня в контракте 10 функций, мне в AllowedMessages 10 имен писать через | ? выглядит как-то избыточно  // using unions to represent available messages // this allows processing them with pattern matching type AllowedMessage = IncreaseCounter | ResetCounter  // the main entrypoint: called when a contract receives an message from other contracts fun onInternalMessage(in: InMessage) {     // use `lazy` to defer loading fields until they are accessed     val msg = lazy AllowedMessage.fromSlice(in.body);

Nick: А в чем вопрос собственно? (reply to 322955)

Nick: Вроде читается нормально:   type IncomingMessage =      | MessageA      | MessageB      | MessageC      | MessageD      | MessageE      | MessageF      | MessageG      | MessageH;

Nick: Поставь наверх и сразу с первого взгляда будет понятно какие сообщения обрабатывает контракт.

Petr: Если 10 сообщений то да, все их нужно описать в этом юнион типе (reply to 322955)

Petr: Это полезно так как описывает интерфейс вашего контракта

Viacheslav: а где можно посмотреть стандартные контракты на tolk? jetton / nft / merkle tree И так далее ?

Nick: https://github.com/ton-blockchain/tolk-bench/tree/master/contracts_Tolk (reply to 322961)

Petr: https://github.com/ton-blockchain/tolk-bench (reply to 322961)

Илья: Что делать после компиляции смартконтракта на func? Как его подключить к кошельку?

Lalo: привет есть идеи почему фейлится транзакция? https://tonviewer.com/transaction/5417ff9f102c59078bdbe18664625524b33b436ebffecc5ee6c6118472274ed7

— 2025-09-25 —

ganyamede: подскажите пожалуйста, есть боты которые отправляют уведомления о транзакциях ТОН кошельков (которые я укажу)

I: Доброе утро!   Снова нужна консультация по TON Connect, в реализации python tonutils (https://github.com/nessshon/tonutils/)  Делаю все по мануалу, подключил кошелек. Отправляю транзакцию методом send_transfer, ожидая увидеть в кошельке запрос на подтверждение, но ничего не происходит, исключений не рейзится, просто возвращается rpc_request_id == 0  Подскажите пожалуйста, что я делаю не так?  async def main():     connector = await tc.init_connector()     wallets = await tc.get_wallets()     selected_wallet = wallets[1]      connect_url = await connector.connect_wallet(selected_wallet)     print(connect_url)      async with connector.connect_wallet_context() as response:         if isinstance(response, TonConnectError):             if isinstance(response, UserRejectsError):                 print("The user rejected the connection.")             else:                 print(f"Connection error: {response.message}")         else:             addr = response.account.address.to_str(is_bounceable=False)             print(f"Connected wallet: {response.account.address.to_str(is_bounceable=False)}")      rpc_request_id = await connector.send_transfer(         destination="UQCZq3_Vd21-4y4m7Wc-ej9NFOhh_qvdfAkAYAOHoQ__Ness",         amount=1,         body="Hello from tonutils!",     )     print(f"RPC request ID: {rpc_request_id}")

I: upd: Добавил запрос статуса транзакции, пришло подтверждение. Подтвердил, везде месаджи об успешной транзакции, но самой транзакции нет и быть не могло, потому что баланс 0.      async with connector.pending_request_context(rpc_request_id) as response:         if isinstance(response, TonConnectError):             if isinstance(response, UserRejectsError):                 print("The user rejected the transaction.")             else:                 print(f"Transaction error: {response.message}")         else:             print(f"Transaction sent successfully! Hash: {response.normalized_hash}") (reply to 322973)
